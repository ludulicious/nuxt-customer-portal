...
const forbidden = [
  ...
  { label: 'request-scoped portal runtime', pattern: /(?:PortalRequestContext|runWithPortalRequestContext|enterPortalRequestContext|createRequestAwareAuth|createRequestAwareDatabase)/ },
...
async function sourceFiles(directory: string): Promise<string[]> {
  ...
  for (const entry of entries) {
    if (['node_modules', '.nuxt', '.output', 'dist'].includes(entry.name)) continue
...
assert.deepEqual(violations, [], `SaaS/control-plane code crossed the portal runtime boundary:\n${violations.join('\n')}`)
