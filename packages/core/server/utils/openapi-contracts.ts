...
export const registerPortalOpenApiContracts = (contracts: PortalOpenApiContracts): void => {
  ...
  if (index === -1) registry.push(contracts)
  else registry[index] = contracts
}
...
const addContract = (operation: OpenApiOperation): void => {
  ...
  const querySchema = querySchemas[operationId] ?? registered.find(item => item.query?.[operationId])?.query?.[operationId]
  if (querySchema) {
    operation.parameters = [...((operation.parameters ?? []) as unknown[]), ...queryParameters(querySchema)]
  }
  const bodySchema = bodySchemas[operationId] ?? registered.find(item => item.body?.[operationId])?.body?.[operationId]
  if (bodySchema) {
