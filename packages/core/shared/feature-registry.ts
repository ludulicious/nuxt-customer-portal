...
export const mergePortalModuleMenuContributions = (
  ...
  modules.map((module) => {
    const additions = contributions
      .filter((contribution) => contribution.moduleId === module.id)
      .map((contribution) => contribution.item)
...
export const sortPortalDashboardWidgets = <
  T extends { id: string; area: keyof typeof dashboardAreaOrder; order: number }
>(
  widgets: readonly T[]
): T[] =>
  [...widgets].sort(
    (left, right) =>
      dashboardAreaOrder[left.area] - dashboardAreaOrder[right.area] ||
