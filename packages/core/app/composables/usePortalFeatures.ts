...
export const usePortalFeatures = () => {
  ...
  const featureEnabled = (feature: PortalFeatureDefinition) => {
    ...
    if (!['timesheets', 'invoices', 'service-requests', 'invoice-timesheets'].includes(feature.id)) return true
  ...
  const navigation = computed(() => features.value.filter(featureEnabled)
  ...
  const clientIntegrations = computed(() => features.value.filter(featureEnabled)
