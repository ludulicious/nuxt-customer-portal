...
test('packages use public package exports rather than host or filesystem contracts', async () => {
  const files = (await walk(join(root, 'packages'))).filter(file => /\.(?:ts|vue)$/.test(file) && !file.includes('/node_modules/') && !file.includes('/test/'))
  ...
  for (const file of files) {
    ...
    for (const match of source.matchAll(/(?:from|import\()\s*['"](\.\.?\/[^'"]+)/g)) {
      ...
      if (relative(join(root, 'packages'), target).startsWith('..')) violations.push(relative(root, file))
      ...
      if (sourcePackage !== targetPackage && !targetPackage.startsWith('..')) violations.push(relative(root, file))
    }
  }
...
test('platform packages do not import optional business features', async () => {
  ...
  for (const packageName of platformPackages) {
    ...
    for (const file of files) {
      const source = await readFile(file, 'utf8')
      if (/@nuxt-customer-portal\/(?:service-requests|timesheets)/.test(source)) violations.push(relative(root, file))
    }
  }
  assert.deepEqual(violations, [])
})
...
test('reusable package runtime and content contain no demo branding', async () => {
  const files = (await walk(join(root, 'packages'))).filter(file => !file.includes('/test/') && !file.includes('/node_modules/') && !file.endsWith('/package.json'))
  const violations: string[] = []
  for (const file of files) {
    const source = await readFile(file, 'utf8')
    if (/Apex\s?Pro|Ludulicious|Facility Services/i.test(source)) violations.push(relative(root, file))
  }
  assert.deepEqual(violations, [])
})
