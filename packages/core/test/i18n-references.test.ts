...
const packageNames = readdirSync(new URL('packages/', repository), { withFileTypes: true })
  .filter((entry) => entry.isDirectory())
  .map((entry) => entry.name)
const appNames = readdirSync(new URL('apps/', repository), { withFileTypes: true })
  .filter((entry) => entry.isDirectory())
  .map((entry) => entry.name)
...
test('all application translation references exist in English and Dutch', () => {
  ...
  for (const locale of ['en', 'nl'] as const) {
    for (const base of [...packageNames.map((name) => `packages/${name}`), ...appNames.map((name) => `apps/${name}`)]) {
      ...
      for (const key of localeKeys(JSON.parse(readFileSync(file, 'utf8')))) {
        keys[locale].add(key)
      }
    }
  }
  ...
  const files = [
    ...packageNames.flatMap((name) => [
      ...sourceFiles(new URL(`packages/${name}/app/`, repository)),
      ...sourceFiles(new URL(`packages/${name}/shared/`, repository))
    ]),
  ...
  for (const file of files) {
    ...
    for (const locale of ['en', 'nl'] as const) {
      for (const key of literalReferences) {
        assert.ok(keys[locale].has(key), `${file.pathname} references missing ${locale} locale key ${key}`)
      }
    }
    ...
    for (const match of source.matchAll(/(?<![\w$])(?:\$t|t)\(\s*`([^`]*?)\$\{/g)) {
      ...
      for (const locale of ['en', 'nl'] as const) {
        assert.ok(
          [...keys[locale]].some((key) => key.startsWith(prefix)),
          `${file.pathname} references empty ${locale} dynamic locale family ${prefix}`
        )
      }
    }
  }
