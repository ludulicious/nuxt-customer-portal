...
export const listGenericClientsPage = async (query: GenericClientListQuery): Promise<ClientListResponse> => {
  const conditions = [
    ...
    query.search
      ? or(
          ...
          ilike(clientProfile.officialName, `%${query.search}%`),
          ilike(organization.slug, `%${query.search}%`)
        )
      : undefined,
    query.moduleId
      ? inArray(
          organization.id,
          db
            .select({ id: clientModule.organizationId })
            .from(clientModule)
            .where(and(eq(clientModule.moduleId, query.moduleId), eq(clientModule.enabled, true)))
        )
      : undefined
  ...
  const [countRow] = await db
    .select({ total: count() })
    .from(organization)
    .innerJoin(clientProfile, eq(clientProfile.organizationId, organization.id))
    .where(where)
  ...
  const rows = await db
    ...
    .innerJoin(clientProfile, eq(clientProfile.organizationId, organization.id))
    .where(where)
    .orderBy(direction(primary), asc(organization.id))
...
export const listSelectableClients = async (moduleId?: string) => {
  const page = await listGenericClientsPage({
    status: 'active',
    moduleId,
    page: 1,
    pageSize: 100,
...
export const updateClient = async (organizationId: string, input: ClientUpdateInput) =>
  db.transaction(async (tx) => {
    ...
    if (Object.keys(orgValues).length) {
      await tx.update(organization).set(orgValues).where(eq(organization.id, organizationId))
    }
    ...
    if (input.registrationNumber !== undefined) {
      values.registrationNumber = normalizeNullable(input.registrationNumber)
    }
    if (input.vatNumber !== undefined) {
      values.vatNumber = normalizeNullable(input.vatNumber)
    }
    if (input.invoiceEmail !== undefined) {
      values.invoiceEmail = normalizeNullable(input.invoiceEmail)
    }
...
export const setClientArchived = async (organizationId: string, actorUserId: string, archived: boolean) => {
  const [updated] = await db
    ...
    .returning({ organizationId: clientProfile.organizationId })
  if (!updated) {
    throw createError({ statusCode: 404, message: 'Client not found' })
  }
}

export const setClientModule = async (
  organizationId: string,
  actorUserId: string,
  moduleId: string,
  enabled: boolean
  ...
  if (selected.archivedAt && enabled) {
    throw createError({ statusCode: 409, message: 'Archived clients cannot enable modules' })
  }
  await db
    .insert(clientModule)
    .values({
      ...
      enabled,
      enabledAt: new Date(),
      enabledById: actorUserId,
      disabledAt: enabled ? null : new Date(),
      disabledById: enabled ? null : actorUserId
    })
    .onConflictDoUpdate({
...
export const requireClientModuleEnabled = async (organizationId: string, moduleId: string) => {
  const [enabled] = await db
    ...
    .from(clientModule)
    .innerJoin(clientProfile, eq(clientProfile.organizationId, clientModule.organizationId))
    .where(
      and(
        eq(clientModule.organizationId, organizationId),
        eq(clientModule.moduleId, moduleId),
    ...
    .limit(1)
  if (!enabled) {
    throw createError({ statusCode: 403, message: `Module ${moduleId} is not enabled for this client` })
  }
}
...
export const getGenericClientDeletionEligibility = async (organizationId: string) => {
  ...
  return {
    ...
    canDelete: selected.members.length === 0 && selected.modules.length === 0,
    memberCount: selected.members.length,
    moduleCount: selected.modules.length
  }
}

export const deleteGenericClient = async (organizationId: string, clientName: string) =>
  db.transaction(async (tx) => {
    const [selected] = await tx
    ...
    const [moduleCount] = await tx
      .select({ modules: count() })
      .from(clientModule)
      .where(eq(clientModule.organizationId, organizationId))
    if (Number(memberCount?.members ?? 0) || Number(moduleCount?.modules ?? 0)) {
      throw createError({ statusCode: 409, message: 'Client still has members or module history' })
    }
