...
const inviteMember = async (client: GenericClientDto) => { busy.value = true; try { await api.invite(client.id, inviteEmail.value, inviteRole.value); inviteEmail.value = ''; inviteRole.value = 'member'; toast.add({ title: t('features.clients.invited'), color: 'success' }) } finally { busy.value = false } }
...
<template>
  <div class="h-full min-h-0 overflow-y-auto">
    <div class="mx-auto flex max-w-[1440px] flex-col gap-4 p-4 sm:p-6 lg:p-8">
      ...
      <div v-if="result && result.pagination.totalPages > 1" class="flex items-center justify-between gap-3"><p class="text-sm text-muted">{{ t('features.clients.resultCount', { count: result.pagination.totalItems }) }}</p><UPagination :page="page" :total="result.pagination.totalItems" :items-per-page="20" @update:page="goToPage" /></div>
    </div>
  </div>
</template>
