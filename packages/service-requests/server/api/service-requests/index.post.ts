...
defineRouteMeta({
  openAPI: {
    ...
    description:
      'Create a service request. Scoped to the active organization and the applicable Service Requests permission.'
  }
})
...
export default defineEventHandler(async (event) => {
  ...
  if (!clientOrganizationId) {
    throw createError({ statusCode: 400, message: 'Client is required' })
  }
