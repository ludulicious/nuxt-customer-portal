...
export const getServiceRequestDashboard = async (
  organizationId: string,
  canManage: boolean,
  scope?: { clientOrganizationId?: string; createdById?: string }
): Promise<ServiceRequestDashboardDto> => {
  const rows = await db
    .select({ request: serviceRequest, clientName: clientOrganization.name })
    .from(serviceRequest)
    .innerJoin(clientOrganization, eq(clientOrganization.id, serviceRequest.clientOrganizationId))
  ...
  const attentionItems = [...active].sort(
    (left, right) =>
...
export const updateServiceRequest = async (id: string, values: Partial<NewServiceRequestRecord>) => {
