...
export const useServiceRequestList = (options: {
  endpoint: string
  filterKeys?: string[]
  ...
  const resource = usePaginatedResource<ServiceRequest, Record<string, string | undefined>>({
    pageSize,
    getKey: (item) => (item as { id: string }).id,
    fetchPage: async ({ filters: query, page, pageSize, signal }) => {
      const result = await $fetch<ServiceRequestListResponse>(options.endpoint, {
  ...
  const load = async (page = currentPage.value) => {
    const result = await resource.loadPage(query(), { page, pageSize }).catch(() => undefined)
    if (result) {
      const lastPage = Math.max(1, result.pagination.pageCount)
      if (page > lastPage) {
        currentPage.value = lastPage
        await syncRoute(lastPage)
        return resource.loadPage(query(), { page: lastPage, pageSize }).catch(() => undefined)
      }
  ...
  const loadNext = async () => {
    const result = await resource.loadNextPage(query())?.catch(() => undefined)
    if (result) {
      currentPage.value = result.pagination.page
  ...
  const loadPrevious = async () => {
    const result = await resource.loadPreviousPage(query())?.catch(() => undefined)
    if (result) {
      currentPage.value = result.pagination.page
      await syncRoute(currentPage.value)
    }
