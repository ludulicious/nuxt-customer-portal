...
import type { Product, Asset } from '../../shared/types'
...
<template>
  <div ref="root" class="scroll-mt-24 rounded-lg border border-primary p-4 sm:p-6" @keydown.esc.stop="emit('cancel')">
    <UForm ref="form" :state="state" :schema="schema" novalidate class="space-y-5" @submit="save"
      ><div class="flex justify-between">
        <h2 class="text-lg font-semibold">{{ t(product ? 'products.edit' : 'products.new') }}</h2>
        <UButton
          icon="i-lucide-x"
          color="neutral"
          variant="ghost"
      ...
      <div class="grid gap-4 sm:grid-cols-2">
        ...
        ><UFormField name="category" :label="t('products.category')"
          ><UInput v-model="state.category" class="w-full" /></UFormField
      ...
      <fieldset v-for="lang in ['en', 'nl'] as const" :key="lang" class="space-y-3 border-t pt-4">
        <legend class="font-medium">{{ lang === 'en' ? 'English' : 'Nederlands' }}</legend>
        <UFormField :name="`content.${lang}.title`" :label="t('products.name')"
          ><UInput v-model="state.content[lang].title" class="w-full" /></UFormField
        ><UFormField :name="`content.${lang}.summary`" :label="t('products.summary')"
          ><UTextarea v-model="state.content[lang].summary" class="w-full" /></UFormField
        ><UFormField
          :name="`content.${lang}.description`"
          :label="t('products.description')"
          :help="t('products.markdownHelp')"
          ><UTextarea v-model="state.content[lang].description" :rows="8" class="w-full" /></UFormField
        ><UFormField :name="`nextSteps.${lang}`" :label="t('products.nextSteps')"
          ><UTextarea v-model="state.nextSteps[lang]" class="w-full"
        /></UFormField>
      </fieldset>
