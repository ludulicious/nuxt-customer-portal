...
}>('/api/organizations/email-provider', { query: organizationQuery })
...
const save = async () => {
  busy.value = true
  try {
    await $fetch('/api/organizations/email-provider', {
      method: 'PUT',
      query: organizationQuery.value,
    ...
    toast.add({
      title: t(
        permissionRequired ? 'organization.emailProvider.permissionRequired' : 'organization.emailProvider.invalid'
      ),
      description: permissionRequired
        ? t('organization.emailProvider.permissionRequiredDescription')
        : t('organization.emailProvider.invalidDescription'),
      color: 'error'
    })
  } finally {
    busy.value = false
...
const recheck = async () => {
  busy.value = true
  try {
    status.value = await $fetch('/api/organizations/email-provider', {
      query: { ...organizationQuery.value, refresh: '1' }
    })
  } catch (error) {
  ...
  <UCard>
    ...
    <div class="space-y-4">
      ...
      <UAlert
        ...
        :title="
          t(status?.configured ? 'organization.emailProvider.configured' : 'organization.emailProvider.notConfigured')
        "
        :description="
          status?.configured
            ? t('organization.emailProvider.masked', { suffix: status.keyLastFour })
            : t('organization.emailProvider.required')
        "
        variant="outline"
      />
      ...
      <UForm :state="{ apiKey }" :schema="schema" class="space-y-3" @submit="save">
        <UFormField
          name="apiKey"
          :label="
            t(status?.configured ? 'organization.emailProvider.replaceLabel' : 'organization.emailProvider.keyLabel')
          "
        >
        ...
        <div class="flex flex-wrap gap-2">
          <UButton type="submit" icon="i-lucide-save" :loading="busy">
            {{ t(status?.configured ? 'organization.emailProvider.replace' : 'organization.emailProvider.configure') }}
          </UButton>
          <UButton
    ...
    <ConfirmationModal
      ...
      :title="t('organization.emailProvider.removeTitle')"
      :message="t('organization.emailProvider.removeDescription')"
      :confirm-text="t('organization.emailProvider.remove')"
