...
import {
  configureOrganizationEmailCredential,
...
defineRouteMeta({
  openAPI: {
    ...
    description:
      'Save organization email credentials. Uses the current authenticated session and enforces the relevant portal permissions.'
  }
})
...
export default defineEventHandler(async (event) => {
  ...
  try {
    return await configureOrganizationEmailCredential(
      organizationId,
      session.user.id,
      schema.parse(await readBody(event)).apiKey
    )
  } catch (error) {
    if (error instanceof EmailProviderRejectedError) {
      ...
      throw createError({
        ...
        message: permissionRequired
          ? 'The Resend API key is restricted to sending email'
          : 'The Resend API key could not be validated',
