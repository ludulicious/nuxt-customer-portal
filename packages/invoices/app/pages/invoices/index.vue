...
const listing = useInvoicesAdminList<ClientInvoiceSummaryDto>({
  endpoint: '/api/invoices/client/invoices',
  filterKeys: ['accessId', 'status', 'overdue'],
...
const filters = computed(() => [
  {
    key: 'status',
    placeholder: t('features.invoices.clientInvoices.statusFilter'),
    items: [
      { label: t('features.invoices.clientInvoices.allStatuses'), value: undefined },
      { label: t('features.invoices.admin.invoiceStatus.issued'), value: 'ISSUED' },
      { label: t('features.invoices.admin.invoiceStatus.paid'), value: 'PAID' }
    ]
  },
  {
    key: 'accessId',
    placeholder: t('features.invoices.clientInvoices.supplierFilter'),
    items: [
      { label: t('features.invoices.clientInvoices.allSuppliers'), value: undefined },
      ...(suppliers.value ?? []).map((item) => ({ label: item.providerName, value: item.id }))
    ]
  },
  {
    key: 'overdue',
    placeholder: t('features.invoices.clientInvoices.paymentFilter'),
    items: [
      { label: t('features.invoices.clientInvoices.allPayments'), value: undefined },
      { label: t('features.invoices.clientInvoices.overdueOnly'), value: 'true' },
      { label: t('features.invoices.clientInvoices.notOverdue'), value: 'false' }
    ]
  }
])
  ...
  <InvoicesPageShell v-else class="flex h-full min-h-0 flex-col overflow-hidden">
    <header class="shrink-0 border-b border-default pb-5">
      <div class="flex items-center gap-2">
        ...
        <h1 class="text-2xl font-semibold">{{ t('features.invoices.clientInvoices.title') }}</h1>
      </div>
      <p class="mt-1 hidden text-sm text-muted sm:block">{{ t('features.invoices.clientInvoices.subtitle') }}</p>
    </header>
      ...
      >
        ...
        <div v-else class="grid gap-3">
          <UCard v-for="item in listing.items.value" :key="item.id" class="transition-colors hover:ring-primary/50">
            <NuxtLink
              :to="`/invoices/${item.id}`"
              class="grid gap-4 rounded-lg focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-primary sm:grid-cols-[minmax(12rem,1fr)_minmax(10rem,1fr)_9rem_auto] sm:items-center"
            >
              <div class="min-w-0">
                <p class="truncate font-semibold">{{ item.number }}</p>
                <p class="mt-1 truncate text-sm text-muted">{{ item.supplierName }}</p>
              </div>
              <div>
                <p class="text-sm">{{ date(item.issueDate) }}</p>
                <p class="mt-1 text-xs text-muted">
                  {{ t('features.invoices.admin.dueDate') }} {{ date(item.dueDate) }}
                </p>
              </div>
              <div>
                <p class="font-medium">{{ money(item.totalMinor, item.currency) }}</p>
                <p v-if="item.outstandingMinor" class="mt-1 text-xs text-muted">
                  {{ t('features.invoices.admin.outstanding') }} {{ money(item.outstandingMinor, item.currency) }}
                </p>
              </div>
              <div class="flex items-center justify-end gap-2">
                <UBadge :color="statusColor(item.status)" variant="subtle">{{
                  t(`features.invoices.admin.invoiceStatus.${item.status.toLowerCase()}`)
                }}</UBadge
