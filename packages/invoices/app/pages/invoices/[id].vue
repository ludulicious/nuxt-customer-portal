...
const {
  ...
  error,
  refresh
} = await useAsyncData(`client-invoice-${invoiceId.value}`, () => api.getClientInvoice(invoiceId.value))
useSeoMeta({
  title: computed(() =>
    invoice.value
      ? t('features.invoices.admin.invoiceTitle', { number: invoice.value.number })
      : t('features.invoices.clientInvoices.title')
  )
})
</script>
