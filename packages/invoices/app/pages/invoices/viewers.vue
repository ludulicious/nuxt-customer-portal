...
const { data: viewers, refresh } = await useAsyncData('invoice-viewers', async () =>
  supplier.value ? api.clientInvoiceViewers(supplier.value.id) : []
)
const fixedViewers = computed(() => viewers.value?.filter((person) => person.fixedAccess) ?? [])
const configurableViewers = computed(() => viewers.value?.filter((person) => !person.fixedAccess) ?? [])
const viewerCount = computed(() => viewers.value?.filter((person) => person.assigned).length ?? 0)
    ...
    <section v-else class="my-6 rounded-lg border border-default bg-default">
      ...
      <div v-if="fixedViewers.length" class="border-b border-default">
        ...
        <div class="divide-y divide-default">
          <div v-for="person in fixedViewers" :key="person.id" class="flex items-center justify-between gap-3 p-4">
            <span class="min-w-0"
      ...
      <p v-if="!fixedViewers.length && !configurableViewers.length" class="p-5 text-sm text-muted">
