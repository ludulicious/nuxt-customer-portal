...
const schema = computed(() =>
  z.object({
    ...
    cc: z
      .string()
      .refine(
        (value) => !value.trim() || value.split(',').every((item) => z.string().email().safeParse(item.trim()).success),
        t('features.invoices.validation.validEmail')
      ),
    locale: z.enum(['nl', 'en']),
...
const titleKey = computed(() =>
  props.mode === 'issue'
    ? 'features.invoices.admin.issueAndSend'
    : props.mode === 'reminder'
      ? 'features.invoices.admin.sendReminder'
      : 'features.invoices.admin.resendInvoice'
)
const successKey = computed(() =>
  props.mode === 'issue'
    ? 'features.invoices.admin.invoiceIssuedAndSent'
    : props.mode === 'reminder'
      ? 'features.invoices.admin.reminderSent'
      : 'features.invoices.admin.invoiceResent'
)
...
const send = async () => {
  ...
  try {
    ...
    if (props.mode === 'issue') {
      await api.issueAndSendInvoice(props.invoiceId, input)
    } else if (props.mode === 'reminder') {
      await api.sendInvoiceReminder(props.invoiceId, input)
    } else {
      await api.resendInvoice(props.invoiceId, input)
    }
    await props.refresh()
      ...
      <UForm v-else :state="draft" :schema="schema" class="space-y-4" @submit="send">
        ...
        >
          <template #description>
            ...
            <UButton
              v-if="canConfigureEmailProvider"
        ...
        <UAlert
          v-else-if="preview && !preview.senderDomainVerified"
          color="error"
          icon="i-lucide-shield-alert"
          :title="t('features.invoices.admin.emailDomainUnverified')"
          variant="outline"
        >
          <template #description>
            <p>{{ preview.senderDomain }}</p>
            <UButton
              ...
              class="mt-2"
            >
              {{ t('features.invoices.admin.emailDomainVerificationHelp') }}
            </UButton>
          </template>
        </UAlert>
        <UAlert
          v-if="preview && preview.totalAttachmentSize > preview.maximumAttachmentSize"
          color="error"
          icon="i-lucide-file-warning"
          :title="t('features.invoices.admin.emailAttachmentsTooLarge')"
          variant="outline"
        />
        <div class="grid gap-3 sm:grid-cols-2">
          <UFormField name="to" :label="t('features.invoices.admin.recipientEmail')">
            <UInput v-model="draft.to" type="email" class="w-full" /> </UFormField
          ><UFormField name="cc" label="CC">
            <UInput v-model="draft.cc" :placeholder="t('features.invoices.admin.ccPlaceholder')" class="w-full" />
          </UFormField>
        </div>
        ...
        <UFormField name="body" :label="t('features.invoices.admin.emailBody')">
          <UTextarea v-model="draft.body" :rows="8" class="w-full" />
        </UFormField>
        <div>
          <p class="text-sm font-medium">{{ t('features.invoices.admin.emailAttachments') }}</p>
          <ul class="mt-2 space-y-1 text-sm text-muted">
            <li v-for="file in preview?.attachments" :key="file.fileName">
        ...
        <div class="flex justify-end gap-2">
          ...
          ><UButton
            ...
            :loading="busy"
            :disabled="
              !preview?.emailProviderConfigured ||
              !preview?.senderDomainVerified ||
              (preview?.totalAttachmentSize ?? 0) > (preview?.maximumAttachmentSize ?? 0)
            "
          >
            {{
              t(
                props.mode === 'reminder' ? 'features.invoices.admin.sendReminder' : 'features.invoices.admin.sendEmail'
              )
            }}
          </UButton>
        </div>
