...
const listing = useInvoicesAdminList<InvoiceDto>({ endpoint: '/api/invoices/admin/invoices', filterKeys: ['status', 'clientOrganizationId', 'overdue'], defaultSort: 'issueDate', defaultSortDir: 'desc' })
const invoiceFilters = computed(() => [
  { key: 'status', placeholder: t('features.invoices.admin.list.statusFilter'), items: [{ label: t('features.invoices.admin.list.allStatuses'), value: undefined }, ...(['DRAFT', 'ISSUED', 'PAID', 'VOID'] as const).map(status => ({ label: t(`features.invoices.admin.invoiceStatus.${status.toLowerCase()}`), value: status }))] },
  { key: 'clientOrganizationId', placeholder: t('features.invoices.admin.list.clientFilter'), items: [{ label: t('features.invoices.admin.list.allClients'), value: undefined }, ...props.data.clients.map(client => ({ label: client.name, value: client.organizationId }))] },
  { key: 'overdue', placeholder: t('features.invoices.admin.list.overdueFilter'), items: [{ label: t('features.invoices.admin.list.allPaymentStates'), value: undefined }, { label: t('features.invoices.admin.list.overdue'), value: 'true' }, { label: t('features.invoices.admin.list.notOverdue'), value: 'false' }] }
])
const sortOptions = computed(() => [{ label: t('features.invoices.admin.list.sortIssueDate'), value: 'issueDate' }, { label: t('features.invoices.admin.list.sortDueDate'), value: 'dueDate' }, { label: t('features.invoices.admin.list.sortNumber'), value: 'number' }, { label: t('features.invoices.admin.list.sortTotal'), value: 'totalMinor' }])
...
const periodEntries = computed(() => invoiceableEntries.value.filter(entry => entry.date >= periodFrom.value && entry.date <= periodTo.value))
...
const availableProjects = computed(() => [...new Map(clientEntries.value.map(entry => [entry.projectId, { id: entry.projectId, name: entry.project }])).values()])
const selectedEntries = computed(() => clientEntries.value.filter(entry => selectedProjectIds.value.includes(entry.projectId)))
...
const refreshAvailableProviders = async () => {
  const checks = await Promise.all(providers.value.map(async provider => ({ provider, result: await provider.load({ from: periodFrom.value, to: periodTo.value }) })))
  availableProviders.value = checks.filter(item => item.result.enabled).map(item => item.provider)
}
const chooseClient = (id: string) => { model.clientOrganizationId = id; model.contactId = ''; selectedProjectIds.value = []; const projects = availableProjects.value; if (projects.length === 1) selectedProjectIds.value = [projects[0]!.id] }
...
const chooseTimeSource = async () => { const provider = availableProviders.value[0]; if (!provider) return; const result = await provider.load({ from: periodFrom.value, to: periodTo.value }); if (!result.enabled) { await refreshAvailableProviders(); return }; invoiceableEntries.value = result.entries; source.value = 'TIME'; next() }
const save = async () => { busy.value = true; try { const input = { ...model, contactId: model.contactId || null }; if (source.value === 'TIME' && availableProviders.value[0]) await availableProviders.value[0].create(input); else await api.createInvoice(input); reset(); await props.refresh(); await listing.refresh(); toast.add({ title: t('features.invoices.admin.invoiceCreated'), color: 'success' }) } catch (error) { toast.add({ title: t('features.invoices.messages.saveError'), description: String(error), color: 'error' }) } finally { busy.value = false } }
...
<template>
  <section class="-mx-1 flex h-full min-h-0 flex-col gap-4 px-1" :class="formOpen ? 'overflow-y-auto' : 'overflow-hidden'">
    <InvoicesAdminListToolbar v-model:search="listing.search.value" :filters="invoiceFilters" :filter-values="listing.filters" :sort-options="sortOptions" :sort-by="listing.sortBy.value" :sort-dir="listing.sortDir.value" @filter="listing.setFilter" @sort="listing.sortBy.value = $event" @toggle-direction="listing.toggleSortDir" />
    <InvoicesAdminEmptyState v-if="!formOpen && !senderInvoiceDetailsComplete" icon="i-lucide-file-warning" :title="t('features.invoices.admin.senderDetailsRequiredTitle')" :description="t('features.invoices.admin.senderDetailsRequiredDescription')" :action-label="t('features.invoices.admin.completeSenderDetails')" action-icon="i-lucide-settings-2" @action="openWizard" />
    <InvoicesAdminEmptyState v-else-if="!formOpen && !listing.items.value.length && !listing.pending.value" icon="i-lucide-file-text" :title="t('features.invoices.admin.noInvoicesTitle')" :description="t('features.invoices.admin.noInvoicesDescription')" :action-label="t('features.invoices.admin.createFirstInvoice')" @action="openWizard" />

    <UCard v-if="formOpen" class="scroll-mt-6">
      <template #header>
        <div class="flex items-center justify-between gap-4">
          <h2 class="font-semibold">{{ step === 5 ? t('features.invoices.admin.newInvoiceFor', { name: selectedClient?.name }) : t('features.invoices.admin.newInvoice') }}</h2>
          <span class="shrink-0 text-sm text-muted">{{ t('features.invoices.admin.wizardStep', { step, total: 5 }) }}</span>
        </div>
      </template>

      <div v-if="step === 1" class="grid gap-3 md:grid-cols-2">
        <button class="invoice-source-card min-w-0 rounded-lg border border-default p-5 text-left hover:border-primary" @click="source = 'FREE'; next()"><UIcon name="i-lucide-file-pen-line" class="mb-3 size-6" /><h3 class="font-medium">{{ t('features.invoices.admin.freeForm') }}</h3><p class="mt-1 text-sm text-muted">{{ t('features.invoices.admin.freeFormDescription') }}</p></button>
        <button v-if="availableProviders.length" class="invoice-source-card min-w-0 rounded-lg border border-default p-5 text-left hover:border-primary" @click="chooseTimeSource"><UIcon name="i-lucide-clock-3" class="mb-3 size-6" /><h3 class="font-medium">{{ t(availableProviders[0]!.labelKey) }}</h3><p class="mt-1 text-sm text-muted">{{ t(availableProviders[0]!.descriptionKey) }}</p></button>
      </div>
      <div v-else-if="step === 2" class="space-y-4">
        <div v-if="source === 'TIME'" class="grid gap-3 sm:grid-cols-2">
          <UFormField :label="t('features.invoices.admin.periodFrom')"><UInput v-model="periodFrom" type="date" class="w-full" /></UFormField>
          <UFormField :label="t('features.invoices.admin.periodTo')"><UInput v-model="periodTo" type="date" class="w-full" /></UFormField>
        </div>
        ...
        <div class="grid gap-2">
          ...
          <p v-if="source === 'TIME' && !clientOptions.length" class="text-sm text-muted">{{ t('features.invoices.admin.noInvoiceableTimeInPeriod') }}</p>
        </div>
        <div v-if="selectedClient && !selectedClientReady" class="rounded-lg border border-warning/50 bg-warning/10 p-4">
          <div class="flex items-start gap-3">
            ...
            <div class="min-w-0 flex-1">
              ...
              <p class="mt-1 text-sm text-muted">{{ t('features.invoices.admin.clientDetailsRequiredDescription', { fields: missingClientInvoiceDetails.join(', ') }) }}</p>
      ...
      <div v-else-if="step === 3" class="space-y-3"><h3 class="font-medium">{{ t('features.invoices.admin.selectProjects') }}</h3><label v-for="project in availableProjects" :key="project.id" class="flex items-center gap-3 rounded-lg border border-default p-4"><input v-model="selectedProjectIds" type="checkbox" :value="project.id"><span class="flex-1">{{ project.name }}</span><span>{{ money(clientEntries.filter(entry => entry.project === project.name).reduce((sum, entry) => sum + entry.amountMinor, 0)) }}</span></label></div>
      <div v-else-if="step === 4" class="space-y-4"><UFormField :label="t('features.invoices.admin.detailLevel')"><USelect v-model="detailLevel" :items="[{ label: t('features.invoices.admin.summary'), value: 'SUMMARY' }, { label: t('features.invoices.admin.detailed'), value: 'DETAILED' }]" value-key="value" class="w-full" /></UFormField><UFormField v-if="detailLevel === 'SUMMARY'" :label="t('features.invoices.admin.groupBy')"><USelect v-model="groupBy" :items="[{ label: t('features.invoices.admin.perProject'), value: 'PROJECT' }, { label: t('features.invoices.admin.perPerson'), value: 'PERSON' }, { label: t('features.invoices.admin.perActivity'), value: 'ACTIVITY' }, { label: t('features.invoices.admin.perPersonActivity'), value: 'PERSON_ACTIVITY' }]" value-key="value" class="w-full" /></UFormField></div>
      ...
      <UForm v-else :state="model" :schema="invoiceSchema" class="invoice-workbench" @submit="save">
        <div class="invoice-meta-grid">
          <div class="invoice-meta-column">
            <UFormField name="number" :label="t('features.invoices.admin.invoiceNumber')" required>
              <div class="flex items-center gap-3"><UInput v-model="model.number" class="min-w-0 flex-1" /><span class="hidden whitespace-nowrap text-xs text-muted sm:block">{{ t('features.invoices.admin.lastUsedInvoiceNumber', { number: lastInvoiceNumber }) }}</span></div>
            </UFormField>
            <UFormField name="issueDate" :label="t('features.invoices.admin.invoiceDate')"><UInput v-model="model.issueDate" type="date" class="w-full" /></UFormField>
            <UFormField name="dueDate" :label="t('features.invoices.admin.dueDate')"><UInput v-model="model.dueDate" type="date" class="w-full" /></UFormField>
          </div>
          <div class="invoice-meta-column">
            <UFormField :label="t('features.invoices.admin.client')"><UInput :model-value="selectedClient?.name" disabled class="w-full" /></UFormField>
            <UFormField name="contactId" :label="t('features.invoices.admin.contact')"><USelect v-model="model.contactId" :items="selectedClient?.contacts.map(item => ({ label: `${item.name} · ${item.email}`, value: item.id })) ?? []" value-key="value" class="w-full" /></UFormField>
            <UFormField :label="t('features.invoices.admin.currency')"><UInput v-model="model.currency" disabled class="w-full" /></UFormField>
          </div>
        </div>

        <UFormField name="subject" :label="t('features.invoices.admin.subject')"><UInput v-model="model.subject" class="w-full" /></UFormField>
        ...
        <section class="invoice-ledger" :aria-label="t('features.invoices.admin.invoiceDetails')">
          <div class="invoice-ledger-head" aria-hidden="true">
            <span>{{ t('features.invoices.admin.description') }}</span><span>{{ t('features.invoices.admin.quantity') }}</span><span>{{ t('features.invoices.admin.unitPrice') }}</span><span>{{ t('features.invoices.admin.amount') }}</span><span>{{ t('features.invoices.admin.vat') }}</span><span />
          </div>
          <div v-for="(line, index) in model.lines" :key="index" class="invoice-line">
            <UFormField class="invoice-line-description" :name="`lines.${index}.description`" :label="t('features.invoices.admin.description')"><UTextarea v-model="line.description" :rows="1" autoresize class="w-full" /></UFormField>
            <UFormField class="invoice-line-quantity" :name="`lines.${index}.quantityMilli`" :label="t('features.invoices.admin.quantity')"><UInputNumber :model-value="line.quantityMilli / 1000" :min="0.001" :step="0.001" :format-options="quantityFormat" :increment="false" :decrement="false" :ui="{ base: 'text-right' }" class="w-full" @update:model-value="updateQuantity(line, $event)" /></UFormField>
            <UFormField class="invoice-line-price" :name="`lines.${index}.unitPriceMinor`" :label="t('features.invoices.admin.unitPrice')"><UInputNumber :model-value="line.unitPriceMinor / 100" :min="0" :step="0.01" :format-options="currencyFormat" :increment="false" :decrement="false" :ui="{ base: 'text-right' }" class="w-full" @update:model-value="updateUnitPrice(line, $event)" /></UFormField>
            <div class="invoice-line-amount"><span class="invoice-mobile-label">{{ t('features.invoices.admin.amount') }}</span><strong>{{ money(lineAmount(line), model.currency) }}</strong></div>
            <UFormField class="invoice-line-vat" :name="`lines.${index}.vatRateBasisPoints`" :label="t('features.invoices.admin.vat')"><UInputNumber :model-value="line.vatRateBasisPoints / 10_000" :min="0" :max="1" :step="0.0001" :format-options="percentageFormat" :increment="false" :decrement="false" :ui="{ base: 'text-right' }" class="w-full" @update:model-value="updateVatRate(line, $event)" /></UFormField>
            <UButton v-if="source === 'FREE' || !line.timeEntryIds.length" type="button" class="invoice-line-remove self-start justify-self-end" style="transform: translateY(8px)" size="xs" color="neutral" variant="ghost" icon="i-lucide-trash-2" :aria-label="t('features.invoices.admin.removeInvoiceLine', { number: index + 1 })" @click="model.lines.splice(index, 1)" />
          </div>
          <div class="invoice-ledger-footer">
            <div class="invoice-add-line"><UButton type="button" size="sm" variant="outline" icon="i-lucide-plus" @click="model.lines.push(emptyLine())">{{ t('features.invoices.admin.addLine') }}</UButton></div>
            <dl class="invoice-totals">
              <div><dt>{{ t('features.invoices.admin.subtotal') }}</dt><dd>{{ money(subtotalMinor, model.currency) }}</dd></div>
              <div><dt>{{ t('features.invoices.admin.vat') }}</dt><dd>{{ money(vatMinor, model.currency) }}</dd></div>
              <div class="invoice-total"><dt>{{ t('features.invoices.admin.totalPayable') }}</dt><dd>{{ money(totalMinor, model.currency) }}</dd></div>
            </dl>
          </div>
        </section>

        <UFormField name="notes" :label="t('features.invoices.admin.notes')"><UTextarea v-model="model.notes" :rows="5" class="w-full" /></UFormField>
        <footer class="invoice-actions">
          <UButton type="button" color="neutral" variant="outline" @click="back">{{ t('features.invoices.admin.previous') }}</UButton>
          <UButton type="submit" icon="i-lucide-file-check-2" :loading="busy">{{ t('features.invoices.admin.createDraftInvoice') }}</UButton>
        </footer>
      </UForm>

      <div v-if="step < 5" class="mt-6 flex justify-between"><UButton color="neutral" variant="outline" @click="step === 1 ? reset() : back()">{{ t(step === 1 ? 'features.invoices.cancel' : 'features.invoices.admin.previous') }}</UButton><UButton v-if="step > 1" :disabled="!canContinue" @click="next">{{ t('features.invoices.admin.next') }}</UButton></div>
    </UCard>

    <InvoicesAdminPaginatedList v-if="!formOpen && listing.items.value.length" class="min-h-0 flex-1" :pagination="listing.pagination.value" :pending="listing.pending.value" :loading-next="listing.loadingNextPage.value" :loading-previous="listing.loadingPreviousPage.value" :has-next="listing.hasNextPage.value" :has-previous="listing.hasPreviousPage.value" @next="listing.loadNext" @previous="listing.loadPrevious" @page="listing.goToPage">
    ...
    <UCard v-for="invoice in listing.items.value" :key="invoice.id" :class="invoice.isOverdue ? 'ring-1 ring-warning/60' : ''" class="transition-colors hover:ring-1 hover:ring-primary/50">
      <div class="grid grid-cols-[minmax(0,1fr)_2.75rem] items-center gap-3">
        <NuxtLink :to="`/admin/invoices/${invoice.id}`" class="group min-w-0 flex-1 rounded-lg focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-primary">
          <div class="flex flex-wrap items-center gap-2"><p class="min-w-0 break-words font-semibold">{{ invoice.number }} · {{ invoice.recipientName }}</p><UBadge :color="statusColor(invoice.status)" variant="subtle">{{ t(`features.invoices.admin.invoiceStatus.${invoice.status.toLowerCase()}`) }}</UBadge><UBadge v-if="invoice.isOverdue" color="warning" variant="subtle">{{ t('features.invoices.admin.overdueDays', { count: invoice.daysOverdue }) }}</UBadge></div>
          <p class="mt-1 whitespace-normal break-words text-sm text-muted">{{ t('features.invoices.admin.dueDate') }} {{ invoice.dueDate }} · {{ money(invoice.totalMinor, invoice.currency) }} · {{ t('features.invoices.admin.outstanding') }} {{ money(invoice.outstandingMinor, invoice.currency) }}</p>
          <p v-if="invoice.isOverdue" class="mt-1 text-xs text-warning">{{ t('features.invoices.admin.reminderSummary', { count: invoice.reminderCount, date: invoice.lastReminderSentAt ? dateTime(invoice.lastReminderSentAt) : t('features.invoices.admin.never') }) }}</p>
        </NuxtLink>
        <NuxtLink :to="`/admin/invoices/${invoice.id}`" :aria-label="t('features.invoices.admin.openInvoice', { number: invoice.number })" class="grid size-11 shrink-0 place-items-center justify-self-end rounded focus-visible:outline-2 focus-visible:outline-primary"><UIcon name="i-lucide-chevron-right" class="size-5 text-muted" /></NuxtLink>
      </div>
    </UCard>
      </div>
...
.invoice-meta-grid { display: grid; gap: 1.5rem; padding-block-end: 1.5rem; border-block-end: 1px solid var(--invoices-rule); }
.invoice-meta-column { display: grid; gap: 1rem; align-content: start; }
.invoice-ledger { overflow: clip; border: 1px solid var(--invoices-rule); border-radius: var(--invoices-radius); }
.invoice-ledger-head { display: none; }
.invoice-line { display: grid; grid-template-columns: minmax(0, 1fr) minmax(0, 1fr); gap: 0.75rem; padding: 1rem; border-block-end: 1px solid var(--invoices-rule); }
...
.invoice-total { padding-block-start: 0.75rem; border-block-start: 1px solid var(--invoices-rule); font-weight: 700; }
.invoice-actions { display: flex; flex-wrap: wrap; justify-content: flex-end; gap: 0.75rem; padding-block-start: 1rem; border-block-start: 1px solid var(--invoices-rule); }
@media (min-width: 75rem) {
  ...
  .invoice-ledger-head { display: grid; gap: 0.75rem; padding: 0.75rem 1rem; border-block-end: 1px solid var(--invoices-rule); font-size: 0.75rem; color: var(--ui-text-muted); }
