...
import type { InvoicesListPagination } from '@nuxt-customer-portal/invoices/shared/types/invoice'

const props = defineProps<{ pagination: InvoicesListPagination, pending: boolean, loadingNext: boolean, loadingPrevious: boolean, hasNext: boolean, hasPrevious: boolean, initialScrollTop?: number }>()
...
<template>
  <div class="flex h-full min-h-0 flex-col">
    <div ref="scrollContainer" class="invoices-list-scroll min-h-0 flex-1 overflow-y-auto overflow-x-hidden" @scroll="emit('scroll', scrollContainer?.scrollTop ?? 0)">
    ...
    <footer class="flex min-h-12 shrink-0 flex-wrap items-center justify-between gap-3 border-t border-default px-1 pt-3 text-sm text-muted">
      <span>{{ $t('features.invoices.admin.list.totalRecords', pagination.total) }}</span>
