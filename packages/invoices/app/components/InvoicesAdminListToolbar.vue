...
<template>
  <div ref="toolbar" class="shrink-0 border-y border-default py-2">
    <div class="flex items-center gap-2">
      <UInput
        v-model="search"
        :placeholder="t('features.invoices.admin.list.search')"
      ...
      <template v-if="!compact">
        ...
        <UButton
          ...
          :aria-label="t('features.invoices.admin.list.direction')"
      ...
      <template v-else>
        ...
        <UButton
          ...
          :aria-label="t('features.invoices.admin.list.sort')"
          @click="showSort = true"
        />
      </template>
    </div>
    <UModal v-model:open="showFilters" :title="t('features.invoices.admin.list.filters')">
      <template #body>
        <div class="space-y-4">
    ...
    <UModal v-model:open="showSort" :title="t('features.invoices.admin.list.sort')">
      <template #body>
        <div class="space-y-4">
          <USelect
            :model-value="sortBy"
          ...
          /><UButton
            ...
            @click="emit('toggleDirection')"
          >
            {{ t('features.invoices.admin.list.direction') }}
          </UButton>
        </div>
      </template>
    </UModal>
