...
defineRouteMeta({
  openAPI: {
    ...
    description:
      'Upload an invoice attachment. Scoped to the active organization and the applicable Invoices permission.'
  }
})
...
export default defineEventHandler(async (event) => {
  ...
  if (!file?.filename) {
    throw createError({ statusCode: 400, message: 'Select a file to attach' })
  }
