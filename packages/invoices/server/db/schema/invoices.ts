...
export const invoiceEmailDeliveryStatus = invoicesSchema.enum('invoice_email_delivery_status', [
  'PENDING',
  'SENT',
  'FAILED'
])
...
export const billingContact = invoicesSchema.table(
  'billing_contact',
  {
  ...
  (table) => [
    uniqueIndex('billing_contact_org_email_uidx').on(table.organizationId, table.email),
    index('billing_contact_org_idx').on(table.organizationId)
  ]
)

export const invoiceClientAccess = invoicesSchema.table(
  'client_access',
  {
    id: text('id').primaryKey(),
  ...
  (table) => [
    uniqueIndex('invoice_client_access_provider_client_uidx').on(
      table.providerOrganizationId,
      table.clientOrganizationId
    ),
    index('invoice_client_access_client_idx').on(table.clientOrganizationId)
  ]
)

export const invoiceClientViewer = invoicesSchema.table(
  'client_viewer',
  {
    id: text('id').primaryKey(),
    accessId: text('access_id')
      .notNull()
      .references(() => invoiceClientAccess.id, { onDelete: 'cascade' }),
    userId: text('user_id')
      .notNull()
  ...
  (table) => [
    uniqueIndex('invoice_client_viewer_access_user_uidx').on(table.accessId, table.userId),
    index('invoice_client_viewer_user_idx').on(table.userId)
  ]
)

export const invoice = invoicesSchema.table(
  'invoice',
  {
    id: text('id').primaryKey(),
    organizationId: text('organization_id')
...
export const invoiceLine = invoicesSchema.table(
  'invoice_line',
  {
    id: text('id').primaryKey(),
...
export const invoicePayment = invoicesSchema.table(
  'invoice_payment',
  {
    id: text('id').primaryKey(),
    invoiceId: text('invoice_id')
...
export const invoiceHistory = invoicesSchema.table(
  'invoice_history',
  {
    id: text('id').primaryKey(),
    invoiceId: text('invoice_id')
...
export const invoiceAttachment = invoicesSchema.table(
  'invoice_attachment',
  {
    id: text('id').primaryKey(),
    invoiceId: text('invoice_id')
...
export const invoiceEmailDelivery = invoicesSchema.table(
  ...
  (table) => [
    ...
    uniqueIndex('invoice_email_delivery_pending_invoice_uidx')
      .on(table.invoiceId)
