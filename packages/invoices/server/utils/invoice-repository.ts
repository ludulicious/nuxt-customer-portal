...
const overdue = (dueDate: string, status: string, outstandingMinor: number) => {
  if (!['ISSUED'].includes(status) || outstandingMinor <= 0) {
...
export const ensureInvoiceSettings = async (organizationId: string) => {
  ...
  return (
    await db.select().from(invoiceSettings).where(eq(invoiceSettings.organizationId, organizationId)).limit(1)
  )[0]!
}
...
export const requireInvoicesEnabled = async (organizationId: string) => {
  ...
  if (!settings.enabled) {
    throw createError({ statusCode: 403, message: 'Invoices are not enabled for this organization' })
  }
  return settings
}
...
export const updateInvoiceSettings = async (
  ...
  input: Omit<InvoiceSettingsDto, 'organizationId'>
) => {
  const [updated] = await db
    .insert(invoiceSettings)
    .values({ organizationId, ...input })
    .onConflictDoUpdate({ target: invoiceSettings.organizationId, set: { ...input, updatedAt: new Date() } })
    .returning()
  return updated!
}
...
export const listInvoiceClients = async (_organizationId: string) => {
  const clients = await listSelectableClients()
  const ids = clients.map((item) => item.organizationId)
  const contacts = ids.length
    ? await db.select().from(billingContact).where(inArray(billingContact.organizationId, ids))
    : []
...
export const listBillingContacts = async (clientOrganizationId: string) => {
  ...
  return db
    .select()
    .from(billingContact)
    .where(eq(billingContact.organizationId, clientOrganizationId))
...
export const createBillingContact = async (
  ...
  try {
    const [created] = await db
      ...
      .values({ id: nanoid(), organizationId: clientOrganizationId, ...input })
      .returning()
    return created!
  } catch (error) {
    if (
      ...
      throw createError({ statusCode: 409, message: 'A billing contact with this email already exists' })
    }
    throw error
  }
}
...
export const updateBillingContact = async (
  ...
  try {
    const [updated] = await db
      ...
      .where(and(eq(billingContact.id, id), eq(billingContact.organizationId, clientOrganizationId)))
      .returning()
    if (!updated) {
      throw createError({ statusCode: 404, message: 'Billing contact not found' })
    }
    return updated
  } catch (error) {
    if (
...
export const ensureClientAccess = async (providerOrganizationId: string, clientOrganizationId: string) => {
  const [created] = await db
    .insert(invoiceClientAccess)
    .values({ id: nanoid(), providerOrganizationId, clientOrganizationId })
    .onConflictDoUpdate({
      target: [invoiceClientAccess.providerOrganizationId, invoiceClientAccess.clientOrganizationId],
...
const requireClientInvoicesModule = async (clientOrganizationId: string) => {
  const [enabled] = await db
    ...
    .limit(1)
  if (!enabled) {
    throw createError({ statusCode: 403, message: 'Invoices are not enabled for this client organization' })
  }
}
...
export const listClientInvoiceSuppliers = async (clientOrganizationId: string, userId: string, isAdmin: boolean) => {
  ...
  const accesses = await db
    ...
    .where(
      and(eq(invoiceClientAccess.clientOrganizationId, clientOrganizationId), eq(invoiceClientAccess.enabled, true))
    )
  const assignments =
    accesses.length && !isAdmin
      ? await db
          .select()
          .from(invoiceClientViewer)
          .where(
            and(
              inArray(
                invoiceClientViewer.accessId,
                accesses.map((item) => item.id)
              ),
              eq(invoiceClientViewer.userId, userId)
            )
          )
      : []
  const allowed = accesses.filter(
    (item) => isAdmin || assignments.some((assignment) => assignment.accessId === item.id)
  )
  const providers = await getPortalOrganizationsByIds(allowed.map((item) => item.providerOrganizationId))
  return allowed.map((item) => ({
    id: item.id,
    providerOrganizationId: item.providerOrganizationId,
    providerName: providers.find((provider) => provider.id === item.providerOrganizationId)?.name ?? '',
    canViewInvoices: true
  }))
}

const requireClientAccess = async (
  clientOrganizationId: string,
  userId: string,
  isAdmin: boolean,
  invoiceId?: string
  ...
  const [selected] = await db
    ...
    .where(
      and(
        ...
        eq(invoice.clientOrganizationId, clientOrganizationId),
        inArray(invoice.status, ['ISSUED', 'PAID'])
      )
    )
    .limit(1)
...
export const listClientInvoices = async (clientOrganizationId: string, userId: string, isAdmin: boolean) => {
  ...
  const results = await Promise.all(
    accesses.map(async (access) =>
      (await listInvoices(access.providerOrganizationId))
        .filter(
          (item) => item.clientOrganizationId === clientOrganizationId && ['ISSUED', 'PAID'].includes(item.status)
        )
        .map((item) => ({
          id: item.id,
          number: item.number,
          ...
          supplierName:
            providers.find((provider) => provider.id === access.providerOrganizationId)?.name ?? item.senderName,
          accessId: access.id
        }))
    )
  )
...
export const getClientInvoice = async (clientOrganizationId: string, userId: string, isAdmin: boolean, id: string) => {
  ...
  const {
    payments: _payments,
    reminderCount: _reminders,
    lastReminderSentAt: _lastReminder,
    history: _history,
    emailDeliveries: _deliveries,
    ...safe
  } = selected
  return { ...safe, supplierName: providers[0]?.name ?? selected.senderName, accessId: access.id }
}

export const getClientInvoiceAttachment = async (
  clientOrganizationId: string,
  userId: string,
  isAdmin: boolean,
  invoiceId: string,
...
export const listClientInvoiceViewers = async (accessId: string, clientOrganizationId: string) => {
  const [access] = await db
    ...
    .where(
      and(
        eq(invoiceClientAccess.id, accessId),
        eq(invoiceClientAccess.clientOrganizationId, clientOrganizationId),
        eq(invoiceClientAccess.enabled, true)
      )
    )
    .limit(1)
  if (!access) {
    throw createError({ statusCode: 404, message: 'Invoice supplier not found' })
  }
  const [members, assignments] = await Promise.all([
    listPortalOrganizationMembers(clientOrganizationId),
    db.select().from(invoiceClientViewer).where(eq(invoiceClientViewer.accessId, accessId))
  ])
  return members.map((member) => ({
    ...
    assigned:
      ['owner', 'admin'].includes(member.organizationRole) || assignments.some((item) => item.userId === member.id),
    fixedAccess: ['owner', 'admin'].includes(member.organizationRole)
  }))
}

export const setClientInvoiceViewer = async (
  accessId: string,
  clientOrganizationId: string,
  actorUserId: string,
  userId: string,
  ...
  if (!selected) {
    throw createError({ statusCode: 400, message: 'User is not a client member' })
  }
  if (selected.fixedAccess) {
    throw createError({ statusCode: 400, message: 'Organization owners and admins always have invoice access' })
  }
  if (assigned) {
    await db
      .insert(invoiceClientViewer)
      .values({ id: nanoid(), accessId, userId, createdById: actorUserId })
      .onConflictDoNothing()
  } else {
    await db
      .delete(invoiceClientViewer)
      .where(and(eq(invoiceClientViewer.accessId, accessId), eq(invoiceClientViewer.userId, userId)))
  }
...
const totals = (
  ...
  const subtotalMinor = lines.reduce(
    (sum, line) => sum + Math.round((line.quantityMilli * line.unitPriceMinor) / 1000),
    0
  )
...
export const listInvoices = async (organizationId: string): Promise<InvoiceDto[]> => {
  ...
  const [lines, payments, reminders] = await Promise.all([
  ...
  return rows.map((row) => {
    const selectedLines = lines.filter((item) => item.invoiceId === row.id)
    const selectedPayments = payments.filter((item) => item.invoiceId === row.id)
    const amounts = totals(selectedLines, selectedPayments)
    const sent = reminders.filter((item) => item.invoiceId === row.id)
    return {
      ...row,
      ...
      lines: selectedLines.map((line) => ({
        ...line,
        amountMinor: Math.round((line.quantityMilli * line.unitPriceMinor) / 1000)
      })),
      payments: selectedPayments.map(({ id, paidOn, amountMinor, reference, note }) => ({
        id,
        paidOn,
        amountMinor,
        reference,
      ...
      ...amounts,
      ...overdue(row.dueDate, row.status, amounts.outstandingMinor),
      reminderCount: sent.length,
      lastReminderSentAt: sent[0]?.sentAt?.toISOString() ?? null,
      history: [],
      attachments: [],
      emailDeliveries: []
    }
...
export const getInvoice = async (organizationId: string, id: string): Promise<InvoiceDto> => {
  ...
  const [histories, attachments, deliveries] = await Promise.all([
    db
      .select({ history: invoiceHistory, actorName: user.name })
      .from(invoiceHistory)
    ...
    db
      .select({ attachment: invoiceAttachment, uploaderName: user.name })
      .from(invoiceAttachment)
      .innerJoin(user, eq(user.id, invoiceAttachment.uploadedById))
      .where(eq(invoiceAttachment.invoiceId, id))
      .orderBy(desc(invoiceAttachment.createdAt)),
    db
      .select({ delivery: invoiceEmailDelivery, actorName: user.name })
      .from(invoiceEmailDelivery)
      .innerJoin(user, eq(user.id, invoiceEmailDelivery.actorUserId))
      .where(eq(invoiceEmailDelivery.invoiceId, id))
      .orderBy(desc(invoiceEmailDelivery.createdAt))
  ])
  return {
    ...selected,
    history: histories.map(({ history, actorName }) => ({
      ...history,
      actorName,
      createdAt: history.createdAt.toISOString()
    })),
    attachments: attachments.map(({ attachment, uploaderName }) => ({
      id: attachment.id,
    ...
    emailDeliveries: deliveries.map(({ delivery, actorName }) => ({
      ...delivery,
      actorName,
      ccEmails: JSON.parse(delivery.ccEmails),
...
export const getNextInvoiceNumber = async (organizationId: string) => {
  ...
  const latest = existing.find((item) => hasNumericInvoiceSequence(item.number))
  let candidate = latest ? incrementInvoiceNumber(latest.number) : firstInvoiceNumber()
...
export const createInvoiceInTransaction = async (
  ...
  const [duplicate] = await tx
    ...
    .from(invoice)
    .where(and(eq(invoice.organizationId, organizationId), eq(invoice.number, input.number)))
    .limit(1)
  if (duplicate) {
    throw createError({ statusCode: 409, message: 'Invoice number already exists' })
  }
  const client = await getClient(input.clientOrganizationId)
  if (!client || client.archivedAt) {
    throw createError({ statusCode: 400, message: 'Client is unavailable' })
  }
  ...
  const contacts = await db
    .select()
    .from(billingContact)
    .where(eq(billingContact.organizationId, input.clientOrganizationId))
  const contact = input.contactId ? contacts.find((item) => item.id === input.contactId) : undefined
  if (input.contactId && !contact) {
    throw createError({ statusCode: 400, message: 'Contact does not belong to this client' })
  }
  ...
  for (const [position, line] of lines.entries()) {
    await tx.insert(invoiceLine).values({ id: nanoid(), invoiceId, position, ...line })
  }
  await tx
    .insert(invoiceClientAccess)
    .values({ id: nanoid(), providerOrganizationId: organizationId, clientOrganizationId: input.clientOrganizationId })
    .onConflictDoUpdate({
      target: [invoiceClientAccess.providerOrganizationId, invoiceClientAccess.clientOrganizationId],
      set: { enabled: true, updatedAt: new Date() }
    })
...
export const createInvoice = async (organizationId: string, actorUserId: string, input: InvoiceCreateInput) => {
  await requireInvoicesEnabled(organizationId)
  return db.transaction((tx) => createInvoiceInTransaction(tx, organizationId, actorUserId, input))
}
...
export const changeInvoiceStatus = async (
  ...
  db.transaction(async (tx) => {
    ...
    if (action === 'VOID' && !['DRAFT', 'ISSUED'].includes(current.status)) {
      throw createError({ statusCode: 409, message: 'This invoice cannot be voided' })
    }
    if (action === 'UNVOID' && current.status !== 'VOID') {
      throw createError({ statusCode: 409, message: 'Only voided invoices can be unvoided' })
    }
    const status = action === 'VOID' ? 'VOID' : current.issuedAt ? 'ISSUED' : 'DRAFT'
    const [updated] = await tx.update(invoice).set({ status }).where(eq(invoice.id, id)).returning()
    await tx
      .insert(invoiceHistory)
      .values({ id: nanoid(), invoiceId: id, action: action === 'VOID' ? 'VOIDED' : 'UNVOIDED', actorUserId })
    return updated!
  })
export const updateInvoice = async (
  ...
  actorUserId: string,
  id: string,
  input: { number: string; issueDate: string; dueDate: string; subject?: string | null; notes?: string | null }
) =>
  db.transaction(async (tx) => {
    const [current] = await tx
      .select()
    ...
    if (current.status !== 'DRAFT') {
      throw createError({ statusCode: 409, message: 'Only draft invoices can be edited' })
    }
    const [updated] = await tx.update(invoice).set(input).where(eq(invoice.id, id)).returning()
...
export const registerInvoicePayment = async (
  ...
  actorUserId: string,
  id: string,
  input: { paidOn: string; amountMinor: number; reference?: string | null; note?: string | null }
) =>
  db.transaction(async (tx) => {
    const selected = await getInvoice(organizationId, id)
    if (!['ISSUED', 'PAID'].includes(selected.status) || input.amountMinor > selected.outstandingMinor) {
      throw createError({ statusCode: 409, message: 'Payment is not valid for this invoice' })
    }
    const [created] = await tx
      .insert(invoicePayment)
      .values({ id: nanoid(), invoiceId: id, createdById: actorUserId, ...input })
      .returning()
    ...
    if (input.amountMinor === selected.outstandingMinor) {
      await tx.update(invoice).set({ status: 'PAID' }).where(eq(invoice.id, id))
    }
    return created!
  })
export const addInvoiceAttachment = async (
  organizationId: string,
  actorUserId: string,
  id: string,
  ...
  db.transaction(async (tx) => {
    ...
    await tx
      .insert(invoiceHistory)
      .values({ id: nanoid(), invoiceId: id, action: 'ATTACHMENT_ADDED', actorUserId, attachmentName: file.fileName })
    return created!
  })
export const getInvoiceAttachment = async (organizationId: string, invoiceId: string, attachmentId: string) => {
  const [row] = await db
    .select({ attachment: invoiceAttachment })
    .from(invoiceAttachment)
    .innerJoin(invoice, eq(invoice.id, invoiceAttachment.invoiceId))
    .where(
      and(eq(invoice.organizationId, organizationId), eq(invoice.id, invoiceId), eq(invoiceAttachment.id, attachmentId))
    )
    .limit(1)
  if (!row) {
...
export const deleteInvoiceAttachment = async (
  ...
  db.transaction(async (tx) => {
    const attachment = await getInvoiceAttachment(organizationId, invoiceId, attachmentId)
