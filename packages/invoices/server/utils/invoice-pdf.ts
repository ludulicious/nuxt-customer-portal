...
export async function generateInvoicePdf(invoice: InvoiceDto | ClientInvoiceDto, requestedLocale?: string) {
  ...
  const date = (value: string) =>
    new Intl.DateTimeFormat(locale === 'nl' ? 'nl-NL' : 'en-GB', { dateStyle: 'medium', timeZone: 'UTC' }).format(
  ...
  const text = (value: string, x: number, size = 9, font = regular, color = ink) =>
    page.drawText(normalize(value), { x, y, size, font, color })
  const multiline = (
    value: string,
    x: number,
    maxWidth: number,
  ...
  if (invoice.senderLogo?.startsWith('data:image/')) {
    try {
      ...
      /* A malformed optional logo must not prevent invoice generation. */
    }
  }
  if (!logoDrawn) {
  ...
  for (const [label, value] of [
    ...
    [l.issueDate, date(invoice.issueDate)],
    [l.dueDate, date(invoice.dueDate)]
  ]) {
    text(label!, metaX, 8, regular, muted)
    page.drawText(normalize(value!), {
      x: width - margin - regular.widthOfTextAtSize(normalize(value!), 9),
      y,
      size: 9,
      font: regular,
      color: ink
  ...
  const tableHeader = () => {
    ...
    for (const [label, right] of [
      [l.quantity, cols[2]! - 8],
      [l.unitPrice, cols[3]! - 8],
      [l.vat, cols[4]! - 74],
      [l.amount, cols[4]!]
    ]) {
      page.drawText(label as string, {
        x: (right as number) - bold.widthOfTextAtSize(label as string, 8),
        y,
        size: 8,
        font: bold,
        color: muted
  ...
  for (const line of invoice.lines) {
    ...
    description.forEach((value, index) =>
      page.drawText(value, { x: cols[0]!, y: rowTop - index * 11, size: 8.5, font: regular, color: ink })
    )
    const values = [
    ...
    values.forEach((value, index) =>
      page.drawText(normalize(value), {
        x: rights[index]! - regular.widthOfTextAtSize(normalize(value), 8.5),
        y: rowTop,
        size: 8.5,
        font: regular,
        color: ink
  ...
  const summary: Array<[string, number]> = [
    [l.subtotal, invoice.subtotalMinor],
    [l.vat, invoice.vatMinor]
  ]
  if (invoice.paidMinor) {
    summary.push([l.paid, -invoice.paidMinor])
  }
  for (const [label, value] of summary) {
    text(label, summaryX, 9, regular, muted)
    const formatted = money(value)
    page.drawText(normalize(formatted), {
      x: width - margin - regular.widthOfTextAtSize(normalize(formatted), 9),
      y,
      size: 9,
      font: regular,
      color: ink
  ...
  page.drawText(normalize(total), {
    x: width - margin - bold.widthOfTextAtSize(normalize(total), 10),
    y,
    size: 10,
    font: bold,
    color: ink
  ...
  pages.forEach((pdfPage, index) => {
    ...
    pdfPage.drawText(value, {
      x: width - margin - regular.widthOfTextAtSize(value, 8),
      y: 25,
      size: 8,
      font: regular,
      color: muted
