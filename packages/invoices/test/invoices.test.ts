...
import { billingContactCreateSchema, invoiceCreateSchema, invoiceEmailDeliverySchema, invoiceIssueSchema, invoiceSettingsSchema } from '../server/utils/invoice-validation'
...
test('free-form invoice and lifecycle inputs validate', () => {
  const input = { clientOrganizationId: 'client', number: '2026.0001', currency: 'EUR', issueDate: '2026-08-12', dueDate: '2026-09-11', lines: [{ description: 'Consulting', quantityMilli: 1000, unit: 'hour', unitPriceMinor: 10000, vatRateBasisPoints: 2100 }] }
...
test('settings, billing contacts, email, and numbering validate', () => {
  assert.equal(invoiceSettingsSchema.safeParse({ enabled: true, currency: 'eur', defaultVatRateBasisPoints: 2100, address: '', registrationNumber: null, vatNumber: null, iban: null, bic: null, invoiceEmail: null, invoiceEmailTemplate: null, preferredLocale: 'nl' }).success, true)
  assert.equal(billingContactCreateSchema.parse({ name: 'Billing', email: ' Billing@Example.com ' }).email, 'billing@example.com')
  assert.equal(invoiceEmailDeliverySchema.safeParse({ to: 'billing@example.com', cc: [], locale: 'en', subject: 'Invoice', body: 'Attached.' }).success, true)
...
test('legacy Timesheets invoice routes are absent and migration verifies copied ids', () => {
  ...
  const bridgeMigration = readFileSync(new URL('../../invoice-timesheets/migrations/0000_baseline.sql', import.meta.url), 'utf8')
