...
const { data: reviewers, refresh } = await useAsyncData(
  "timesheet-approval-reviewers",
  async () => (supplier.value ? api.clientReviewers(supplier.value.id) : []),
);
const fixedReviewers = computed(
  () => reviewers.value?.filter((person) => person.fixedAccess) ?? [],
);
const configurableReviewers = computed(
  () => reviewers.value?.filter((person) => !person.fixedAccess) ?? [],
);
const reviewerCount = computed(
  () => reviewers.value?.filter((person) => person.assigned).length ?? 0,
);
    ...
    <section v-else class="my-6 rounded-lg border border-default bg-default">
      ...
      <div v-if="fixedReviewers.length" class="border-b border-default">
        ...
        <div class="divide-y divide-default">
          <div
            v-for="person in fixedReviewers"
            :key="person.id"
      ...
      <p
        v-if="!fixedReviewers.length && !configurableReviewers.length"
