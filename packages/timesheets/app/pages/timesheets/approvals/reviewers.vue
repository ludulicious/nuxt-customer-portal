...
<template>
  <TimesheetsPageShell width="narrow" class="h-full overflow-y-auto">
    ...
    <div v-else class="grid gap-6 py-6 lg:grid-cols-[18rem_minmax(0,1fr)]">
      <nav class="space-y-2" :aria-label="t('features.timesheets.approvals.suppliersLabel')"><button v-for="item in suppliers" :key="item.id" type="button" class="w-full rounded-md border p-3 text-left focus-visible:outline-2 focus-visible:outline-primary" :class="selectedId === item.id ? 'border-primary bg-primary/5' : 'border-default hover:bg-elevated/50'" @click="selectedId = item.id"><span class="font-medium">{{ item.workspaceName }}</span><span class="mt-1 flex gap-2 text-xs text-muted"><span>{{ t('features.timesheets.approvals.reviewerCount', item.reviewerCount) }}</span><span>·</span><span>{{ t('features.timesheets.approvals.pendingShort', { count: item.pendingCount }) }}</span></span><span v-if="item.pendingCount && !item.reviewerCount" class="mt-2 block text-xs font-semibold text-warning">{{ t('features.timesheets.approvals.unassigned') }}</span></button></nav>
