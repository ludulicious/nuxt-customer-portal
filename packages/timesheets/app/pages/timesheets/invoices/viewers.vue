...
<template>
  <TimesheetsPageShell width="narrow" class="h-full overflow-y-auto">
    ...
    <div v-else class="grid gap-6 py-6 lg:grid-cols-[18rem_minmax(0,1fr)]">
      <nav class="space-y-2" :aria-label="t('features.timesheets.clientInvoices.suppliersLabel')"><button v-for="item in suppliers" :key="item.id" type="button" class="w-full rounded-md border p-3 text-left focus-visible:outline-2 focus-visible:outline-primary" :class="selectedId === item.id ? 'border-primary bg-primary/5' : 'border-default hover:bg-elevated/50'" @click="selectedId = item.id"><span class="font-medium">{{ item.workspaceName }}</span><span class="mt-1 block text-xs text-muted">{{ t('features.timesheets.clientInvoices.viewerCount', item.viewerCount) }}</span></button></nav>
