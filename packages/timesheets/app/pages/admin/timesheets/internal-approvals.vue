...
const { data, pending, refresh } = await useAsyncData(
  'timesheets-internal-approval-configuration',
  timesheets.internalApprovalConfiguration
)
    ...
    <header>
      <div class="flex items-center gap-2">
        ...
        <h1 class="text-2xl font-semibold text-highlighted">
          {{ t('features.timesheets.internalApprovals.manageTitle') }}
        </h1>
      </div>
      <p class="mt-1 hidden text-sm text-muted sm:block">
        {{ t('features.timesheets.internalApprovals.manageSubtitle') }}
      </p>
    </header>
    <div v-if="pending && !data" class="flex justify-center py-12 text-muted">
    ...
    <template v-else-if="data">
      <UCard>
        <div class="flex items-center justify-between gap-4">
          <div>
            <p class="font-medium">{{ t('features.timesheets.internalApprovals.enabled') }}</p>
            <p class="text-sm text-muted">{{ t('features.timesheets.internalApprovals.enabledHelp') }}</p>
          </div>
          <USwitch
            :model-value="data.enabled"
            :loading="busyId === 'workspace'"
            :aria-label="t('features.timesheets.internalApprovals.enabled')"
            @update:model-value="saveWorkspace"
          />
        </div>
      </UCard>
      <UInput
        ...
        :placeholder="t('features.timesheets.internalApprovals.search')"
        class="w-full"
      />
      <section class="grid gap-3">
        <UCard v-for="member in members" :key="member.id">
          <div class="grid gap-4 lg:grid-cols-[minmax(0,1fr)_auto_minmax(18rem,1fr)] lg:items-center">
            ...
            <UFormField :label="t('features.timesheets.internalApprovals.required')">
              <USwitch
                :model-value="member.internalApprovalRequired"
                :disabled="!data.enabled || busyId === member.id"
                @update:model-value="saveMember(member.id, $event, $event ? member.approverUserIds : [])"
              />
            </UFormField>
            <UFormField
              :label="t('features.timesheets.internalApprovals.approvers')"
              :description="t('features.timesheets.internalApprovals.approversHelp')"
            >
              <USelectMenu
                :model-value="member.approverUserIds"
                multiple
                :items="optionsFor(member.id)"
                value-key="value"
                class="w-full"
                :disabled="!data.enabled || !member.internalApprovalRequired || busyId === member.id"
                @update:model-value="saveMember(member.id, true, $event)"
              />
            </UFormField>
          </div>
        </UCard>
        <UCard v-if="!members.length">
          <p class="py-8 text-center text-sm text-muted">
            {{ t('features.timesheets.internalApprovals.emptySearch') }}
          </p>
        </UCard>
      </section>
    </template>
