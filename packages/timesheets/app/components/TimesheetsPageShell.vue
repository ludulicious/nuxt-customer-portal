...
const { data: loadedSetupStatus } = await useAsyncData<TimesheetsSetupStatusDto | null>(
  ...
  async () =>
    activeOrganizationType.value === 'PROVIDER' && isOrganizationAdmin.value && !props.setupStatus
      ? await timesheets.setupStatus()
      : null
)
...
<template>
  ...
  >
    <TimesheetsSetupChecklist
      v-if="
        activeOrganizationType === 'PROVIDER' &&
        isOrganizationAdmin &&
        visibleSetupStatus &&
        !visibleSetupStatus.complete
      "
      :status="visibleSetupStatus"
      class="mb-5"
    />
    <slot />
  </component>
