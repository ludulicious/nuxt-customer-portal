...
const createActivity = async () => {
  ...
  if (!form.value.activityTypeIds.includes(activityId)) {
    form.value.activityTypeIds.push(activityId)
  }
  ...
  <UCard>
    ...
    <UForm :state="form" :schema="schema" class="space-y-4" @submit="emit('submit')">
      ...
      <div class="flex items-end gap-2">
        ...
        <UButton
          v-if="data.activities.some((item) => item.active)"
          type="button"
          size="sm"
          variant="outline"
          icon="i-lucide-plus"
          :aria-expanded="activityFormOpen"
          @click="activityFormOpen = !activityFormOpen"
        >
          {{ t('features.timesheets.admin.createActivity') }}
        </UButton>
      </div>
      <div
        v-if="!data.activities.some((item) => item.active) || activityFormOpen"
