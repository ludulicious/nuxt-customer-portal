...
const listing = useTimesheetsAdminList<ProjectDto>({
  endpoint: '/api/timesheets/admin/projects',
  filterKeys: ['clientOrganizationId', 'status'],
...
const projectFilters = computed(() => [
  ...
  {
    key: 'status',
    placeholder: t('features.timesheets.admin.list.projectStatusFilter'),
    items: [
      { label: t('features.timesheets.admin.list.allProjectStatuses'), value: 'ALL' },
      { label: t('features.timesheets.admin.projectStatus.active'), value: 'ACTIVE' },
      { label: t('features.timesheets.admin.projectStatus.archived'), value: 'ARCHIVED' }
    ]
  }
])
const sortOptions = computed(() => [
  { label: t('features.timesheets.admin.list.sortName'), value: 'name' },
  { label: t('features.timesheets.admin.list.sortClient'), value: 'clientName' },
  { label: t('features.timesheets.admin.list.sortStartDate'), value: 'startsOn' }
])
...
const statusLabel = (status: ProjectDto['status']) =>
  t(`features.timesheets.admin.projectStatus.${status.toLowerCase()}`)

const returnPath = computed(() => {
  const query = new URLSearchParams()
  for (const [key, value] of Object.entries(route.query)) {
    const selected = Array.isArray(value) ? value[0] : value
    if (selected != null && key !== 'scroll') {
      query.set(key, String(selected))
    }
  }
  if (listScrollTop.value > 0) {
    query.set('scroll', String(Math.round(listScrollTop.value)))
  }
  const suffix = query.toString()
  return suffix ? `/admin/timesheets/projects?${suffix}` : '/admin/timesheets/projects'
})
const projectDetailTo = (project: { id: string }) => ({
  path: `/admin/timesheets/projects/${project.id}`,
...
const save = async () => {
  busy.value = true
  try {
    const created = await timesheets.createProject({
...
<template>
  <section class="flex h-full min-h-0 flex-col gap-5">
    ...
    <TimesheetsAdminPaginatedList
      ...
      @scroll="listScrollTop = $event"
      @next="listing.loadNext"
      @previous="listing.loadPrevious"
      @page="listing.goToPage"
    >
      <TimesheetsAdminEmptyState
        v-if="!activeProjects.length && !formOpen && !listing.pending.value"
        icon="i-lucide-folder-kanban"
        :title="t('features.timesheets.admin.noProjectsTitle')"
        :description="t('features.timesheets.admin.noProjectsDescription')"
        :action-label="t('features.timesheets.admin.createFirstProject')"
        @action="openNew"
      />
        ...
        >
          <div class="grid grid-cols-[minmax(0,1fr)_2.75rem] items-center gap-3">
            <NuxtLink
              ...
              class="group min-w-0 rounded-lg focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-primary"
            >
              ...
              <div class="mt-3 flex flex-wrap gap-2 text-xs text-muted">
                <span
                  >{{ project.activityTypeIds.length }}
                  {{ t('features.timesheets.admin.activities').toLowerCase() }}</span
                ><span v-if="project.budgetMinutes">· {{ formatHours(project.budgetMinutes) }}</span
                ><span v-if="project.budgetMinor">· {{ formatMoney(project.budgetMinor) }}</span>
              </div>
            </NuxtLink>
            <NuxtLink
              :to="projectDetailTo(project)"
              :aria-label="t('features.timesheets.admin.openProject', { name: project.name })"
