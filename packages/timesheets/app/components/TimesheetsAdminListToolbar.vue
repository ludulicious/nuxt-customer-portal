...
<template>
  <div class="border-y border-default py-2">
    <div class="flex items-center gap-2">
      <UInput
        v-model="search"
        :placeholder="t('features.timesheets.admin.list.search')"
      ...
      <template v-if="!isMobile">
        ...
        <UDropdownMenu
          :items="[
            sortOptions.map((option) => ({
              label: option.label,
              icon: sortBy === option.value ? 'i-lucide-check' : undefined,
              onSelect: () => emit('sort', option.value)
            }))
          ]"
          :content="{ align: 'end' }"
        >
        ...
        <UButton
          ...
          :aria-label="t('features.timesheets.admin.list.direction')"
      ...
      <template v-else>
        <UButton
          ...
          :aria-label="t('features.timesheets.admin.list.filters')"
          @click="showFilters = true"
        />
        <UButton
          variant="outline"
          icon="i-lucide-arrow-down-up"
          :aria-label="t('features.timesheets.admin.list.sort')"
          @click="showSort = true"
        />
      </template>
    </div>
    <UModal v-model:open="showFilters" :title="t('features.timesheets.admin.list.filters')">
      <template #body>
        <div class="space-y-4">
    ...
    <UModal v-model:open="showSort" :title="t('features.timesheets.admin.list.sort')">
      <template #body>
        <div class="space-y-4">
          <USelect
            :model-value="sortBy"
          ...
          /><UButton
            ...
            @click="emit('toggleDirection')"
          >
            {{ t('features.timesheets.admin.list.direction') }}
          </UButton>
        </div>
      </template>
    </UModal>
