...
const itemStatus = (item: ReadonlyItem): { label: string, color: BadgeColor } => ({ label: t(`features.timesheets.clientPortal.${item.status.toLowerCase()}`), color: reviewStatusColor(item.status) })
  ...
  <TimesheetsPageShell class="flex h-full min-h-0 flex-col overflow-hidden">
    ...
    <div v-if="selected" class="min-h-0 flex-1 overflow-y-auto">
      ...
      <article class="overflow-hidden rounded-lg border border-default bg-default">
        ...
        <UAlert v-if="isReview && isApprovalItem(selected) && selected.status === 'PENDING' && selected.canManageReviewers && !selected.hasReviewers" color="warning" icon="i-lucide-triangle-alert" :title="t('features.timesheets.approvals.noReviewers')" :description="t('features.timesheets.approvals.noReviewersDescription')" class="m-5" variant="outline"><template #actions><UButton :to="`/timesheets/approvals/reviewers?supplier=${selected.workspaceClientId}`" size="sm" color="warning">{{ t('features.timesheets.approvals.assignReviewers') }}</UButton></template></UAlert>
