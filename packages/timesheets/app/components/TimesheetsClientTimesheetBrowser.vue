...
import type { ClientApprovalItemDto, ClientReviewStatus, ClientSupplierTimesheetItemDto } from '@nuxt-customer-portal/timesheets/shared/types/timesheet'
...
const endpoint = computed(() => isReview.value ? '/api/timesheets/client/approvals' : '/api/timesheets/client/supplier-timesheets')
const filterKeys = computed(() => isReview.value ? ['workspaceClientId', 'status'] : ['workspaceClientId', 'billingStatus'])
const listing = useTimesheetsAdminList<Item>({ endpoint: endpoint.value, filterKeys: filterKeys.value, defaultSort: 'weekStartsOn', defaultSortDir: 'desc' })
const { data: supplierOptions } = await useAsyncData(`timesheet-client-${props.mode}-suppliers`, () => isReview.value ? api.clientApprovalSuppliers() : api.clientSupplierOptions())
const selectedId = computed(() => typeof route.query[detailQueryKey.value] === 'string' ? String(route.query[detailQueryKey.value]) : '')
const selected = computed(() => listing.items.value.find(item => item.id === selectedId.value) ?? null)
const filters = computed(() => [
  ...(isReview.value ? [{ key: 'status', placeholder: t('features.timesheets.approvals.statusFilter'), items: [{ label: t('features.timesheets.approvals.allStatuses'), value: undefined }, { label: t('features.timesheets.clientPortal.pending'), value: 'PENDING' }, { label: t('features.timesheets.clientPortal.approved'), value: 'APPROVED' }, { label: t('features.timesheets.clientPortal.disputed'), value: 'DISPUTED' }] }] : [{ key: 'billingStatus', placeholder: t('features.timesheets.suppliers.billingFilter'), items: [{ label: t('features.timesheets.suppliers.allBillingStatuses'), value: undefined }, { label: t('features.timesheets.clientBilling.awaitingInvoice'), value: 'AWAITING_INVOICE' }, { label: t('features.timesheets.clientBilling.partiallyInvoiced'), value: 'PARTIALLY_INVOICED' }, { label: t('features.timesheets.clientBilling.invoiced'), value: 'INVOICED' }] }]),
  { key: 'workspaceClientId', placeholder: t('features.timesheets.approvals.supplierFilter'), items: [{ label: t('features.timesheets.approvals.allSuppliers'), value: undefined }, ...(supplierOptions.value ?? []).map(item => ({ label: item.name, value: item.id }))] }
])
const sortOptions = computed(() => [{ label: t('features.timesheets.approvals.sortNewest'), value: 'weekStartsOn' }, { label: t('features.timesheets.approvals.sortSupplier'), value: 'supplierName' }, { label: t('features.timesheets.approvals.sortPerson'), value: 'person' }, { label: t('features.timesheets.approvals.sortHours'), value: 'totalMinutes' }, ...(isReview.value ? [{ label: t('features.timesheets.approvals.sortStatus'), value: 'status' }] : [])])
...
const weekPeriod = (weekStartsOn: string) => {
  ...
  return t('features.timesheets.clientPortal.weekPeriod', { week: weekNumber(weekStartsOn), period: new Intl.DateTimeFormat(locale.value, { day: 'numeric', month: 'long', year: 'numeric' }).formatRange(start, end) })
}
const reviewStatusColor = (value: ClientReviewStatus): BadgeColor => value === 'APPROVED' ? 'success' : value === 'DISPUTED' ? 'error' : 'warning'
...
const itemStatus = (item: ReadonlyItem): { label: string, color: BadgeColor } => 'billingStatus' in item
  ? { label: t(`features.timesheets.clientBilling.${billingKey(item.billingStatus)}`), color: item.billingStatus === 'INVOICED' ? 'success' as const : 'neutral' as const }
  : { label: t(`features.timesheets.clientPortal.${item.status.toLowerCase()}`), color: reviewStatusColor(item.status) }
  ...
  <TimesheetsPageShell class="flex h-full min-h-0 flex-col overflow-hidden">
    ...
    <div v-if="selected" class="min-h-0 flex-1 overflow-y-auto">
      ...
      <article class="overflow-hidden rounded-lg border border-default bg-default">
        <header class="flex flex-wrap items-start justify-between gap-4 border-b border-default p-5"><div><p class="text-sm font-medium text-primary">{{ selected.supplierName }}</p><h2 class="mt-1 text-xl font-semibold">{{ weekPeriod(selected.weekStartsOn) }}</h2><p class="mt-1 text-sm text-muted">{{ selected.person }} · {{ t('features.timesheets.approvals.totalHours', { hours: hours(selected.totalMinutes) }) }}</p></div><UBadge :color="itemStatus(selected).color" variant="subtle">{{ itemStatus(selected).label }}</UBadge></header>
        <UAlert v-if="isReview && 'canManageReviewers' in selected && selected.status === 'PENDING' && selected.canManageReviewers && !selected.hasReviewers" color="warning" icon="i-lucide-triangle-alert" :title="t('features.timesheets.approvals.noReviewers')" :description="t('features.timesheets.approvals.noReviewersDescription')" class="m-5" variant="outline"><template #actions><UButton :to="`/timesheets/approvals/reviewers?supplier=${selected.workspaceClientId}`" size="sm" color="warning">{{ t('features.timesheets.approvals.assignReviewers') }}</UButton></template></UAlert>
        <div class="divide-y divide-default px-5"><div v-for="entry in selected.entries" :key="entry.id" class="grid gap-2 py-4 sm:grid-cols-[7rem_minmax(0,1fr)_5rem]"><span class="text-sm text-muted">{{ formatDate(entry.date) }}</span><div><p class="text-sm font-medium">{{ entry.project }} · {{ entry.activity }}</p><p v-if="entry.note" class="mt-1 text-sm text-muted">{{ entry.note }}</p></div><strong class="text-sm sm:text-right">{{ hours(entry.minutes) }}</strong></div></div>
        ...
        <footer v-if="isReview && 'canAct' in selected && selected.canAct" class="flex justify-end gap-2 border-t border-default bg-elevated/40 p-5"><UButton color="neutral" variant="outline" :disabled="saving" @click="disputeOpen = true">{{ t('features.timesheets.clientPortal.dispute') }}</UButton><UButton icon="i-lucide-check" :loading="saving" @click="act(selected, 'APPROVE')">{{ t('features.timesheets.clientPortal.approve') }}</UButton></footer>
      </article>
      <UModal v-if="isReview && selected && 'canAct' in selected" v-model:open="disputeOpen" :title="t('features.timesheets.approvals.disputeTitle')" :description="t('features.timesheets.approvals.disputeDescription')"><template #body><UTextarea v-model="disputeComment" autofocus :placeholder="t('features.timesheets.clientPortal.comment')" class="w-full" /></template><template #footer><div class="flex w-full justify-end gap-2"><UButton color="neutral" variant="ghost" @click="disputeOpen = false">{{ t('features.timesheets.cancel') }}</UButton><UButton color="error" :loading="saving" :disabled="!disputeComment.trim()" @click="act(selected, 'DISPUTE')">{{ t('features.timesheets.clientPortal.dispute') }}</UButton></div></template></UModal>
    </div>
    <section v-else class="flex min-h-0 flex-1 flex-col gap-5 pt-5">
      ...
      <TimesheetsAdminPaginatedList class="min-h-0 flex-1" :pagination="listing.pagination.value" :pending="listing.pending.value" :loading-next="listing.loadingNextPage.value" :loading-previous="listing.loadingPreviousPage.value" :has-next="listing.hasNextPage.value" :has-previous="listing.hasPreviousPage.value" @next="listing.loadNext" @previous="listing.loadPrevious" @page="listing.goToPage">
        <UAlert v-if="!listing.items.value.length && !listing.pending.value" icon="i-lucide-search-x" :title="t(isReview ? 'features.timesheets.approvals.emptyFiltered' : 'features.timesheets.suppliers.emptyFiltered')" :description="t(isReview ? 'features.timesheets.approvals.emptyFilteredDescription' : 'features.timesheets.suppliers.emptyFilteredDescription')" variant="outline" />
