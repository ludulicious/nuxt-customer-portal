  ...
  <UCard class="h-full">
    <template #header>
      <div class="flex items-center justify-between gap-3">
        <h2 class="font-semibold">{{ t('features.timesheets.dashboard.supplierTimesheets.title') }}</h2>
    ...
    <template v-else-if="data?.supplierTimesheets">
      <p v-if="!data.supplierTimesheets.items.length" class="py-4 text-sm text-muted">
        {{ t('features.timesheets.dashboard.supplierTimesheets.empty') }}
      </p>
      <ul v-else class="divide-y divide-default">
        <li
          v-for="item in data.supplierTimesheets.items"
          :key="item.id"
          class="flex items-center justify-between gap-3 py-3 text-sm"
        >
          <span class="min-w-0"
            ><strong class="block truncate">{{ item.person }}</strong
            ><span class="text-muted">{{ item.supplierName }} · {{ date(item.weekStartsOn) }}</span></span
          ><strong class="shrink-0 tabular-nums">{{ minutes(item.totalMinutes) }}</strong>
        </li>
      </ul>
      <div class="mt-3 text-right">
        <UButton to="/timesheets/suppliers" color="neutral" variant="ghost" trailing-icon="i-lucide-arrow-right">
