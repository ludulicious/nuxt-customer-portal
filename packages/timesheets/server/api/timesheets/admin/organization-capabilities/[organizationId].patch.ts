...
export default defineEventHandler(async (event) => {
  ...
  if (organization.organizationType !== 'PROVIDER' && input.workspaceEnabled) throw createError({ statusCode: 400, message: 'Timesheets workspaces can only be enabled for provider organizations' })
