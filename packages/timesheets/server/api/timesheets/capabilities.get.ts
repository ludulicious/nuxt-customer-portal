...
export default defineEventHandler(async (event) => {
  ...
  const internalApprovals =
    workspaceEnabled && (settings[0]?.internalApprovalsEnabled ?? true)
      ? await listApprovalQueue(organizationId, session.user.id)
      : []
  const hasInternalApprovalAssignments =
    workspaceEnabled &&
    (settings[0]?.internalApprovalsEnabled ?? true) &&
  ...
  return {
    ...
    canReviewClientTimesheets:
      reviewWorkspaces.length > 0 &&
      (isAdmin || reviewWorkspaces.some((item) => item.canReview) || approvals.items.length > 0),
