...
defineRouteMeta({
  openAPI: {
    ...
    description:
      'Start the time tracking timer. Scoped to the active organization and the applicable Timesheets permission.'
  }
})
...
export default defineEventHandler(async (event) => {
  ...
  return startTimer(organizationId, session.user.id, timerStartSchema.parse(await readBody(event)))
})
