...
export const migrateGenericClients = async (config, options = {}) => {
  ...
  return withPool(options.databaseUrl, async (pool) => {
    ...
    try {
      const selected = await client.query('SELECT id, name, slug FROM organization WHERE id = $1 OR slug = $1', [
        provider
      ])
      if (selected.rowCount !== 1) {
        throw new Error(`Expected exactly one organization for --provider ${provider}, found ${selected.rowCount}`)
      }
      ...
      if (options.apply && options.once) {
        const migrationJournal = await client.query(
          `SELECT to_regclass('${migrationSchema}.one_time_migration') AS table_name`
        )
        if (migrationJournal.rows[0]?.table_name) {
          const applied = await client.query(
            `SELECT provider_id, applied_at FROM ${quoteIdentifier(migrationSchema)}."one_time_migration" WHERE migration_key = $1`,
            ['generic-clients-v1']
          )
          if (applied.rowCount) {
            if (applied.rows[0].provider_id !== providerOrganization.id) {
              throw new Error(
                `Generic Clients migration was already applied for provider ${applied.rows[0].provider_id}`
              )
            }
      ...
      const linked = await client.query(
        ...
        WHERE wc.workspace_organization_id = $1 AND o.id <> $1 ORDER BY o.name`,
        [providerOrganization.id]
      )
      const allOthers = await client.query('SELECT id, name, slug FROM organization WHERE id <> $1 ORDER BY name', [
        providerOrganization.id
      ])
      const secondaryWorkspaces = await client.query(
        `SELECT workspace_organization_id AS "organizationId", count(*)::int AS "clientLinks"
        FROM timesheets.workspace_client WHERE workspace_organization_id <> $1
        GROUP BY workspace_organization_id ORDER BY workspace_organization_id`,
        [providerOrganization.id]
      )
      ...
      const legacyInvoiceProfiles = await client.query(
        `SELECT to_regclass('timesheets.organization_invoice_profile') AS table_name`
      )
      ...
      if (!options.backupConfirmed) {
        throw new Error('Applying this migration requires --backup-confirmed')
      }
      ...
      try {
        ...
        await client.query(
          `UPDATE organization SET organization_type = CASE WHEN id = $1 THEN 'PROVIDER' ELSE 'CLIENT' END`,
          [providerOrganization.id]
        )
        await client.query(
          `INSERT INTO clients.client_profile
        ...
        for (const row of linked.rows) {
          for (const moduleId of activations) {
            await client.query(
              `INSERT INTO clients.client_module (id, organization_id, module_id, enabled)
              VALUES (gen_random_uuid()::text, $1, $2, true)
              ON CONFLICT (organization_id, module_id) DO UPDATE SET enabled = true, updated_at = now()`,
              [row.id, moduleId]
            )
          }
        }
        ...
        if (options.once) {
          await client.query(
            `INSERT INTO ${quoteIdentifier(migrationSchema)}."one_time_migration" (migration_key, provider_id) VALUES ($1, $2)`,
            ['generic-clients-v1', providerOrganization.id]
          )
        }
  ...
  return withPool(options.databaseUrl, async (pool) => {
    ...
    try {
      ...
      const existingProvider = await client.query(
        `SELECT id, slug FROM organization WHERE organization_type = 'PROVIDER' LIMIT 1`
      )
      ...
      if (timesheetsSchema.rows[0]?.table_name) {
        await client.query(
          `INSERT INTO timesheets.workspace_settings (organization_id, workspace_enabled)
          VALUES ($1, true)
          ON CONFLICT (organization_id) DO UPDATE SET workspace_enabled = true, updated_at = now()`,
          [organizationId]
        )
      }
      ...
      if (invoicesSchema.rows[0]?.table_name) {
        await client.query(
          `INSERT INTO invoices.settings (organization_id, enabled)
          VALUES ($1, true)
...
export const seedPortalProvider = async (options = {}) => {
  ...
  return withPool(options.databaseUrl, async (pool) => {
    ...
    try {
      ...
      await client.query(
        `INSERT INTO organization (id, name, slug, organization_type, created_at)
        VALUES ($1, $2, $3, 'PROVIDER', now()) ON CONFLICT (slug) DO UPDATE SET name = EXCLUDED.name`,
        [organizationId, name, slug]
      )
...
const packageManifest = async (source, cwd) => {
  ...
  try {
    ...
    throw new Error(`Cannot resolve ${source}/portal-manifest from ${cwd}: ${error.message}`, { cause: error })
  }
...
export const assertCompatiblePortalVersions = (manifests) => {
  const officialVersions = new Set(
    manifests
...
export const resolvePortalManifests = async (config, cwd = process.cwd()) => {
  ...
  for (const moduleId of config.clients?.defaultModules ?? []) {
    if (!supportedClientModules.has(moduleId)) {
      throw new Error(`Configured default client module is not installed or client-aware: ${moduleId}`)
    }
  }
  return sorted
}
...
export const sortPortalManifests = (manifests) => {
  ...
  for (const manifest of manifests) {
    ...
    if (journalTables.has(table)) {
      throw new Error(
        `Provider ids ${journalTables.get(table)} and ${manifest.id} resolve to the same migration journal`
      )
    }
  ...
  for (const manifest of manifests) {
    for (const dependency of manifest.dependsOn ?? []) {
      if (!byId.has(dependency)) {
        throw new Error(`Provider ${manifest.id} depends on missing provider ${dependency}`)
      }
    }
  }
  ...
  const visit = (manifest) => {
    ...
    if (visiting.has(manifest.id)) {
      throw new Error(`Migration dependency cycle includes ${manifest.id}`)
    }
    visiting.add(manifest.id)
    for (const id of manifest.dependsOn ?? []) {
      visit(byId.get(id))
    }
...
const migrationFiles = (manifest) => {
  ...
  const directory = isAbsolute(manifest.migrations) ? manifest.migrations : resolve(manifest.root, manifest.migrations)
  if (!existsSync(directory)) {
    throw new Error(`Migration directory does not exist for ${manifest.id}: ${directory}`)
  }
...
const requireDatabaseUrl = (value) =>
  value ||
...
const appliedMigrations = async (client, manifest) => {
  ...
  const result =
    await client.query(`SELECT name, checksum, package_version AS "packageVersion", applied_at AS "appliedAt"
      ...
      for (const manifest of manifests.filter((item) => item.migrations)) {
        ...
        for (const file of files) {
          ...
          if (row && row.checksum !== file.checksum) {
            throw new Error(`Checksum mismatch: ${manifest.id}/${file.name}`)
          }
        }
        providers.push({
          id: manifest.id,
        ...
        for (const file of files) {
          ...
          try {
            if (manifest.migrationSearchPath?.length) {
              const searchPath = manifest.migrationSearchPath
                .map((schema) => quoteIdentifier(assertString(schema, `${manifest.id} migration search path schema`)))
                .join(', ')
              await client.query(`SET LOCAL search_path TO ${searchPath}`)
            }
            await client.query(file.sql)
            await client.query(
              `INSERT INTO ${quoteIdentifier(migrationSchema)}.${quoteIdentifier(table)} (name, checksum, package_version) VALUES ($1, $2, $3)`,
              [file.name, file.checksum, manifest.version]
            )
            await client.query('COMMIT')
...
const verifyLegacyDatabase = async (client) => {
  ...
  if (!journal.rows[0]?.journal) {
    throw new Error('Legacy Drizzle journal was not found')
  }
  const count = await client.query('SELECT count(*)::int AS count FROM drizzle.__drizzle_migrations')
  if (count.rows[0]?.count !== 22) {
    throw new Error(`Expected the recognized 22-entry legacy journal, found ${count.rows[0]?.count ?? 0}`)
  }
  ...
  return withPool(options.databaseUrl, async (pool) => {
    ...
    try {
      ...
      const mapping = manifests
        ...
        .map((manifest) => ({
          id: manifest.id,
          files: migrationFiles(manifest)
            .filter((file) => file.name === '0000_baseline.sql')
            .map((file) => file.name)
        }))
      if (!options.apply) {
        return { dryRun: true, mapping }
      }
      await client.query('SELECT pg_advisory_lock(hashtext($1))', [lockName])
      try {
        for (const { id, files: adoptedNames } of mapping) {
          ...
          if (!manifest) {
            throw new Error(`Legacy provider manifest is missing: ${id}`)
          }
          const adoptedNameSet = new Set(adoptedNames)
          const files = migrationFiles(manifest).filter((file) => adoptedNameSet.has(file.name))
          const { table, rows } = await appliedMigrations(client, manifest)
          if (rows.length) {
            throw new Error(`Provider ${manifest.id} already has an adoption journal`)
          }
          await client.query('BEGIN')
          try {
            for (const file of files) {
