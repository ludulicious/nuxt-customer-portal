...
const updateUserRole = async (userId: string) => {
  ...
  try {
    ...
    toast.add({
      ...
      description: apiError.message || t('admin.errors.failedToUpdateRole'),
      color: 'error'
    })
  } finally {
    ...
    <UTable v-else :data="users" :columns="columns" :loading="loading">
      ...
      <template #role-cell="{ row }">
        ...
        <div v-else class="flex items-center gap-2">
          ...
          <UButton
            v-if="currentUser?.id !== row.original.id"
                ...
                ? [
                    {
                      ...
                      icon: 'i-lucide-ban',
                      onSelect: () =>
                        row.original.banned ? selectUser(row.original, 'unban') : openBanModal(row.original)
                    }
                  ]
                : []),
              {
                label: t('admin.user.actions.impersonate'),
                icon: 'i-lucide-user-cog',
                onSelect: () => selectUser(row.original, 'impersonate')
              },
              {
                label: t('admin.user.actions.sessions'),
                icon: 'i-lucide-monitor',
                onSelect: () => selectUser(row.original, 'sessions')
              },
              {
                label: t('admin.user.actions.changePassword'),
                icon: 'i-lucide-key',
                onSelect: () => selectUser(row.original, 'password')
              },
              {
                label: t('admin.user.actions.update'),
                icon: 'i-lucide-edit',
                onSelect: () => selectUser(row.original, 'update')
              },
              ...(row.original.role !== 'admin' && currentUser?.id !== row.original.id
                ? [
                    {
                      label: t('admin.user.actions.delete'),
                      icon: 'i-lucide-trash-2',
                      color: 'error' as const,
                      onSelect: () => selectUser(row.original, 'delete')
                    }
                  ]
                : []),
              ...(removableUserIds?.includes(row.original.id)
                ? [
                    {
                      label: t('admin.organization.detail.members.remove'),
                      icon: 'i-lucide-user-minus',
                      color: 'error' as const,
                      onSelect: () => emit('remove', row.original)
                    }
                  ]
                : [])
            ]
