...
const selectedText = computed<PortalEmailText>({
  get: () =>
    selectedItem.value
      ? { ...selectedItem.value.definition.defaults[selectedLocale.value], ...overrides[overrideKey.value] }
      : { subject: '', body: '', footer: '' },
  set: (value) => {
...
const payload = () => ({
  ...
  htmlTemplate: props.settings.htmlTemplate
})
const save = async () => {
  busy.value = true
  try {
    ...
    const result = await $fetch<PortalEmailSettings>(
      ...
      { method: 'PUT', body: { definition: item.definition, text: selectedText.value } }
    )
      ...
      <div class="grid gap-6 lg:grid-cols-[14rem_minmax(0,1fr)]">
        ...
        <div class="space-y-4">
          ...
          <div class="flex flex-wrap gap-2">
            ...
            <UButton type="button" icon="i-lucide-send" :disabled="!testAddress" :loading="busy" @click="sendTest">{{
