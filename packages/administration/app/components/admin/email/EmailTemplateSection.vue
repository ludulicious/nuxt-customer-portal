...
const save = async () => {
  busy.value = true
  try {
    const result = await $fetch<PortalEmailSettings>('/api/admin/email/template', {
      method: 'PUT',
      body: { htmlTemplate: state.htmlTemplate || null }
    })
    ...
    <UCard>
      <template #header>
        <div class="flex items-center justify-between gap-3">
          ...
          <UBadge v-if="usingProjectTemplate" color="neutral" variant="subtle">{{
      ...
      <div class="mt-3 flex justify-end">
        <UButton
          ...
          :loading="busy"
          @click="reset"
          >{{ t('admin.email.resetTemplate') }}</UButton
        >
      </div>
    </UCard>
    <div class="flex justify-end">
      <UButton type="submit" icon="i-lucide-save" :loading="busy">{{ t('admin.email.saveTemplate') }}</UButton>
    </div>
  </UForm>
</template>
