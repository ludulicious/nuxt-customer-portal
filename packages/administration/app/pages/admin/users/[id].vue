...
const saveRole = async () => {
  if (!user.value || isCurrentUser.value || role.value === user.value.role) {
    return
  }
  try {
    ...
    toast.add({
      ...
      description: apiError.message || t('admin.user.updateRole.error'),
      color: 'error'
    })
  } finally {
      ...
      <template v-else-if="user">
          ...
          <div class="flex min-w-0 items-center gap-4">
            ...
            <div class="min-w-0">
              <div class="flex flex-wrap items-center gap-2">
                <h1 class="min-w-0 break-words text-2xl font-semibold text-highlighted">
                ...
                <UBadge :color="user.banned ? 'error' : 'success'" variant="subtle">
                  {{ user.banned ? t('admin.user.status.banned') : t('admin.user.status.notBanned') }}
                </UBadge>
              </div>
            ...
            <dl class="grid gap-x-6 gap-y-5 sm:grid-cols-2">
              ...
              <div v-if="user.banned && user.banExpires" class="sm:col-span-2">
                ...
                <dd class="mt-1 font-medium text-highlighted">
                  {{ new Date(user.banExpires).toLocaleString(locale) }}
                </dd>
              </div>
            </dl>
          </UCard>
          ...
          <UCard>
            ...
            <div class="space-y-4">
              <p class="text-sm text-muted">
                {{ isCurrentUser ? t('admin.user.detail.ownRoleHint') : t('admin.user.updateRole.description') }}
              </p>
        ...
        <UCard>
          ...
          <div class="flex flex-wrap gap-2">
            <UButton icon="i-lucide-monitor" color="neutral" variant="outline" @click="showSessionsModal = true">
              {{ t('admin.user.actions.sessions') }}
            </UButton>
            <UButton icon="i-lucide-key" color="neutral" variant="outline" @click="showPasswordModal = true">
            ...
            >
              {{ t('admin.user.actions.impersonate') }}
            </UButton>
            <UButton
