...
const sortBy = ref<'name' | 'email' | 'createdAt'>(
  ['email', 'createdAt'].includes(String(route.query.sortBy)) ? (route.query.sortBy as 'email' | 'createdAt') : 'name'
)
...
function buildListQuery(includeScroll = false) {
  ...
  if (includeScroll && listScrollTop.value > 0) {
    query.scroll = String(Math.round(listScrollTop.value))
  }
  return query
}
...
const visibleUsers = computed(() => {
  ...
  const filtered = users.value.filter((user) => {
    ...
    const matchesStatus =
      statusFilter.value === 'all' || (statusFilter.value === 'banned' ? Boolean(user.banned) : !user.banned)
...
<template>
  ...
  >
        ...
        <div ref="listToolbar" class="shrink-0 border-b border-default pb-4">
          <div class="flex items-center gap-2">
            <UInput
              ...
              :placeholder="t('admin.user.list.searchPlaceholder')"
              :loading="loading"
              clearable
            />
            ...
            <template #body>
              <div class="space-y-4">
                <UFormField :label="t('admin.user.list.role')">
                  <USelect v-model="roleFilter" :items="roleOptions" value-key="value" class="w-full" /> </UFormField
                ><UFormField :label="t('admin.user.list.status')">
        ...
        <UEmpty
          ...
          :description="
            searchQuery || roleFilter !== 'all' || statusFilter !== 'all'
              ? t('admin.user.list.emptyFiltered')
              : t('admin.user.list.emptyDescription')
          "
          variant="outline"
        >
          <template v-if="!searchQuery && roleFilter === 'all' && statusFilter === 'all'" #actions>
            <UButton icon="i-lucide-plus" size="lg" @click="showCreateModal = true">
              {{ t('admin.user.list.createFirstUser') }}
            </UButton>
          </template>
        </UEmpty>
              ...
              <div class="min-w-0">
                <div class="flex flex-wrap items-center gap-2">
                  <h2 class="truncate font-semibold text-highlighted">
                    {{ user.name || t('admin.user.list.notAvailable') }}
                  </h2>
                  <UBadge :color="user.role === 'admin' ? 'primary' : 'neutral'" variant="subtle">{{
                    t(`admin.user.roles.${user.role || 'user'}`)
                  }}</UBadge>
                ...
                <div class="mt-1 flex flex-wrap items-center gap-x-2 gap-y-1 text-xs text-muted">
                  ...
                  <span
                    >{{ t('admin.user.list.created') }} {{ new Date(user.createdAt).toLocaleDateString(locale) }}</span
                  >
                </div>
              </div>
              <span
    ...
    >
      ...
      <span v-if="visibleUsers.length !== users.length">{{
        t('admin.user.list.totalCount', { count: users.length })
      }}</span>
    </footer>
