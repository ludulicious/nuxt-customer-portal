      ...
      <header class="flex flex-col gap-3 border-b border-default pb-4 sm:flex-row sm:items-end sm:justify-between">
        <div class="flex min-w-0 gap-3">
          ...
          <div class="min-w-0">
            ...
            <p class="text-sm text-muted">{{ t('admin.user.list.description') }}</p>
          </div>
        </div>
        <div class="flex shrink-0 items-center gap-1 self-end sm:self-auto">
          <UButton icon="i-lucide-plus" size="sm" variant="outline" @click="showCreateModal = true">
            <span class="hidden sm:inline">{{ t('admin.user.list.newUser') }}</span>
          </UButton>
