...
export default defineEventHandler(async (event) => {
  ...
  if (!parsed.success) {
    throw createError({ statusCode: 400, message: 'A valid user and organization role are required' })
  }
  ...
  const [existingMember] = await db
    ...
    .from(memberTable)
    .where(and(eq(memberTable.organizationId, organizationId), eq(memberTable.userId, parsed.data.userId)))
    .limit(1)
  if (existingMember) {
