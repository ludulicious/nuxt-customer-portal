...
export default defineEventHandler(async (event) => {
  ...
  if (path.startsWith('/api/auth/')) {
    if (!state.completed && path.includes('/sign-in/social')) {
      throw createError({ statusCode: 403, message: 'Social authentication is unavailable until portal setup is complete' })
    }
    if (!state.completed && path.includes('/sign-up/')) {
      const body = await readBody(event)
      if (!isReservedPortalAdmin(body?.email)) throw createError({ statusCode: 403, message: 'Only the reserved portal administrator can create the first account' })
    }
    return
  }
