...
const image = z.string().max(2_800_000).refine(value => !value || /^data:image\/(png|jpeg|webp);base64,[a-z0-9+/=]+$/i.test(value), 'Use a PNG, JPEG, or WebP image')
const link = z.string().trim().max(500).refine(value => !value || value.startsWith('/') || z.url().safeParse(value).success, 'Use a valid URL or path')
...
const homeSchema = z.object({
  heroTitle: text(160), heroDescription: text(1000), heroActionLabel: text(80), heroActionUrl: link,
  introductionTitle: text(160), introductionBody: text(4000),
  features: z.array(featureSchema).max(6), supportTitle: text(160), supportBody: text(2000), supportVisible: z.boolean()
})
...
export const portalSettingsSchema = z.object({
  ...
  appearance: z.object({
    theme: z.enum(portalThemeNames), colorMode: z.enum(portalColorModePolicies),
    primaryLight: color, primaryDark: color
  }),
  enabledModules: z.array(z.enum(portalModuleIds)).min(1).transform(values => [...new Set(values)]),
  content: z.object({ en: localizedContentSchema, nl: localizedContentSchema })
}).superRefine((value, context) => {
  if (value.enabledModules.includes('invoice-timesheets') && (!value.enabledModules.includes('timesheets') || !value.enabledModules.includes('invoices'))) {
    context.addIssue({ code: 'custom', path: ['enabledModules'], message: 'Invoice from timesheets requires both Timesheets and Invoices' })
  }
})
...
export interface PortalOnboardingState { adminExists: boolean, completed: boolean, step: PortalOnboardingStep }

const defaultLocaleContent = (locale: 'en' | 'nl'): PortalContent => locale === 'nl'
? {
  home: { heroTitle: 'Uw klantenportaal', heroDescription: 'Eén veilige plek voor samenwerking met uw klanten.', heroActionLabel: 'Inloggen', heroActionUrl: '/login', introductionTitle: 'Welkom', introductionBody: 'Configureer deze pagina in de portaalinstellingen.', features: [], supportTitle: 'Ondersteuning', supportBody: 'Neem contact met ons op wanneer u hulp nodig heeft.', supportVisible: true },
...
: {
  home: { heroTitle: 'Your customer portal', heroDescription: 'One secure place to collaborate with your customers.', heroActionLabel: 'Sign in', heroActionUrl: '/login', introductionTitle: 'Welcome', introductionBody: 'Configure this page in the portal settings.', features: [], supportTitle: 'Support', supportBody: 'Contact us whenever you need help.', supportVisible: true },
...
export const defaultPortalSettings = (name = 'Customer Portal'): PortalSettings => ({
  branding: { portalName: name, tagline: 'Customer workspace', supportEmail: '', supportUrl: '', markLight: '', markDark: '', logoLight: '', logoDark: '' },
