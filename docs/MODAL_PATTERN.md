...
<!-- Parent component -->
<template>
  <YourModal v-if="showModal" v-model:open="showModal" :user="selectedUser" @success="handleSuccess" />
</template>
```
...
<template>
  <!-- Trigger button -->
  <UButton @click="openModal(item)"> Open Modal </UButton>

  <!-- Modal component -->
  <YourModal v-if="showModal" v-model:open="showModal" :user="selectedItem" @success="handleSuccess" />
</template>
```
    ...
    <template #body>
      <UForm :state="form" :schema="formSchema" @submit="handleSubmit">
        ...
        <div class="flex gap-4 justify-end pt-4">
          <UButton type="button" variant="outline" @click="open = false"> {{ t('common.cancel') }} </UButton>
          <UButton type="submit" color="error"> {{ t('admin.user.ban.confirm') }} </UButton>
        </div>
      </UForm>
    </template>
  </UModal>
...
<template>
  <UButton @click="openBanModal(user)"> Ban User </UButton>

  <BanUserModal v-if="showBanModal" v-model:open="showBanModal" :user="selectedUser" @success="handleBanSuccess" />
</template>
```
