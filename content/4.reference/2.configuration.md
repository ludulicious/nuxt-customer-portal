...
githubPath: content/4.reference/2.configuration.md
navigation:
...
::warning
The pinned Customer Portal `.env.example` does not yet list `BETTER_AUTH_SECRET`, because Better Auth reads it internally rather than through an application-level `process.env` expression. It is still required in production. Generate a unique value with `openssl rand -base64 32`; never copy a documentation placeholder into a deployment. See the [Better Auth secret reference](https://better-auth.com/docs/reference/options#secret) for the upstream contract.
::
