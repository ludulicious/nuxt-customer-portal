...
githubPath: content/6.guides/9.invoicing.md
navigation:
  icon: i-lucide-receipt-text
---

Organization owners and administrators manage sales invoices at `/admin/timesheets/invoices`. Linked client organizations receive issued invoices through `/timesheets/invoices` when invoice access is enabled.

## Prepare invoice data

Before creating an invoice:

1. Complete sender details under `/admin/timesheets/settings`.
2. Complete the client address and invoice email under `/admin/timesheets/clients`.
...
Enable invoice access on the supplier’s client record. Client organization owners and administrators always have access and can assign additional viewers at `/timesheets/invoices/viewers`.
