...
import type { TimesheetsListResponse } from '#layers/timesheets/shared/types/timesheet'
...
export const useTimesheetsAdminList = <Item>(options: {
  ...
  const resource = usePaginatedResource<Item, Record<string, string | undefined>>({
    ...
    fetchPage: ({ filters: query, page, pageSize, signal }) => $fetch<TimesheetsListResponse<Item>>(options.endpoint, { query: { ...query, page, pageSize }, signal })
  })
  const query = () => ({
