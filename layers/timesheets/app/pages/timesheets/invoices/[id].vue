...
const { data: invoice, pending, error, refresh } = await useAsyncData(`timesheets-client-invoice-${invoiceId.value}`, () => api.getClientInvoice(invoiceId.value))
useSeoMeta({ title: computed(() => invoice.value ? t('features.timesheets.admin.invoiceTitle', { number: invoice.value.number }) : t('features.timesheets.clientInvoices.title')) })
</script>
...
<template>
  <div class="timesheet-admin mx-auto h-full min-h-0 w-full max-w-[1440px] overflow-y-auto px-4 py-5 sm:px-6 sm:py-6 lg:px-8 lg:py-8">
    ...
    <TimesheetsInvoiceDetail v-else-if="invoice" :invoice="invoice" :refresh="refresh" mode="client" />
  </div>
</template>
