...
import { setClientReviewer } from '#layers/timesheets/server/utils/timesheet-repository'
import { clientReviewerUpdateSchema } from '#layers/timesheets/server/utils/timesheet-validation'
...
export default defineEventHandler(async (event) => {
  ...
  return setClientReviewer(getRouterParam(event, 'workspaceClientId')!, organizationId, session.user.id, input.userId, input.assigned)
})
