...
import { auth, generateId } from '#portal/server/utils/auth'
import { db } from '#portal/server/utils/db'
import { invitation as invitationTable, member as memberTable } from '#portal/server/db/schema/auth-schema'
import { eq, and } from 'drizzle-orm'
import { sendEmail } from '#portal/server/utils/email'
import { getInvitationEmailContent } from '#portal/server/utils/email-texts'
...
export default defineEventHandler(async (event) => {
  ...
  try {
    ...
    const { organization: organizationTable } = await import('#portal/server/db/schema/auth-schema')
