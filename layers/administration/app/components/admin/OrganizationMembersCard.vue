...
import type { AdminUserResponse, OrganizationMemberWithUser } from '#types'
...
const removableUserIds = computed(() => props.canRemove
...
const removeMember = async () => {
  ...
  try {
    removing.value = true
    await $fetch(`/api/admin/organizations/${props.organizationId}/members/${selectedMember.value.id}`, { method: 'DELETE' })
    toast.add({ title: t('common.success'), description: t('admin.organization.detail.members.removeSuccess'), color: 'success' })
    emit('refresh')
  } catch (error) {
...
<template>
  <UCard>
    ...
    <ConfirmationModal v-if="showRemoveModal && selectedMember" v-model:open="showRemoveModal"
