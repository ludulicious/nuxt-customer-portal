...
Feature layers register a `PortalFeatureDefinition` containing navigation, dashboard widgets, and an organization-role policy. Client code consumes authentication and active-organization state through `usePortalSession`; server handlers use `#portal/server/portal` for the database, session, tenant, and authorization adapters.
