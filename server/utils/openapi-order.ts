...
const timesheetsSectionRank = (path: string): number => {
  if (path === '/api/timesheets/bootstrap') return 0
  if (path.includes('/api/timesheets/timer')) return 10
  if (path.includes('/api/timesheets/entries')) return 20
  if (path.includes('/api/timesheets/weeks')) return 30
  if (path.includes('/api/timesheets/admin/bootstrap')) return 40
  if (path.includes('/api/timesheets/admin/approvals')) return 100
...
const invoiceActionRank = (path: string): number => {
  if (path.endsWith('/attachments')) return 10
  if (path.includes('/attachments/')) return 20
  if (path.endsWith('/pdf')) return 30
  if (path.endsWith('/issue')) return 40
  if (path.endsWith('/email-preview')) return 50
  if (path.endsWith('/email')) return 60
  if (path.endsWith('/email-status')) return 70
  if (path.endsWith('/reminder-preview')) return 80
  if (path.endsWith('/reminder')) return 90
  if (path.endsWith('/payments')) return 100
  return 0
}
...
const orderMethods = (pathItem: OpenApiPathItem): OpenApiPathItem => Object.fromEntries(
