...
| `organizations` | OWNER profile, organization selection, settings, and email-provider credentials |
| `clients` | CLIENT profiles, memberships, invitations, archival, and module activation |
| `preset` | Composition of core, UI, authentication, organizations, and clients |
| `kit` | Portal configuration, diagnostics, OWNER seeding, provider migrations, and client-model migration |
...
`authentication` owns account entry and recovery screens while core retains Better Auth configuration and identity records. `organizations` owns OWNER settings; `clients` owns CLIENT organizations and their contacts. There is no installation-wide administrator bypass.
...
An installation has exactly one `OWNER`; every other organization is a `CLIENT`. Client-aware features advertise a stable module ID and may contribute module-specific configuration to client details. Disabling a module blocks new client activity without deleting history.
...
4. OWNER settings belong in organizations.
