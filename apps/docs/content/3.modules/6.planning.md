...
Google and Zoom use dedicated OAuth applications and encrypted tokens. Configure the portal-wide `PORTAL_ENCRYPTION_KEY`, provider client IDs and secrets, public callback URLs, and scheduled Nitro tasks before enabling live booking. Existing deployments can retain `PLANNING_ENCRYPTION_KEY` as an override while legacy encrypted tokens remain stored. Sandbox bookings can use internal availability without external connections.
