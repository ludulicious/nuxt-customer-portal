...
githubPath: apps/docs/content/3.modules/3.service-requests.md
navigation:
  icon: i-lucide-ticket-check
---

The `service-requests` layer is an optional example extension for module developers. It is not required for the client-access, timesheet, and invoicing workflow. It owns its UI, API handlers, policies, translations, validation, types, tests, and Drizzle schema.
...
Use it alongside the [create-a-layer guide](/contributing/create-a-layer). The [service-request user guide](/guides/service-requests) documents the current product workflow and its organization-wide visibility. Move to [Timesheets](/modules/timesheets) or [Invoices](/modules/invoices) when you need a larger example.
