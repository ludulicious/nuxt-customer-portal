...
githubPath: apps/docs/content/5.operations/4.observability.md
navigation:
...
The deployed hosts expose a process-level health endpoint at `/api/health`. Structured logging, application metrics, distributed tracing, and bundled error-tracking integration are not currently included.
...
An HTTP probe of `/api/health` confirms that the Nuxt process is listening. It does not prove PostgreSQL, authentication, authorization, OAuth, or email delivery are healthy.
...
Propose this work as a reusable module or core capability in [module proposals](/contributing/propose-a-module).
