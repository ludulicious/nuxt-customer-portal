...
githubPath: apps/docs/content/7.contributing/4.documentation.md
navigation:
  icon: i-lucide-file-pen-line
---

The documentation site lives in `apps/docs` in the [canonical monorepo](https://github.com/ludulicious/nuxt-customer-portal). Product behavior and documentation now change together in one pull request.
...
Documentation pages are Markdown files under `apps/docs/content/`. Numeric prefixes control order but are removed from public routes.
...
Every maintained page has an **Edit page** link to its source and a visible **Report a docs issue** action. Keep `githubPath` aligned with the repository-relative file path, such as `apps/docs/content/3.modules/1.overview.md`, so the edit link remains accurate.
