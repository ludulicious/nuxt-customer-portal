...
| `timesheets` | 78 | `/api/timesheets/**` | Entry, timers, setup, approvals, reporting, client settings, invoices, PDFs, email, and cross-organization access |
...
Do not accept `organizationId` from a normal browser request when the active session already determines the organization. OWNER-side client operations require an active OWNER membership and never use a global administrator bypass.
