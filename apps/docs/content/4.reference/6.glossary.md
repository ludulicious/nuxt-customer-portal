...
githubPath: content/4.reference/6.glossary.md
navigation:
...
The publicly developed Nuxt application that combines portal core, platform layers, and optional business feature layers into a deployable customer-facing portal. An explicit open-source product license is still pending.

### Host application

The root Nuxt application that composes the enabled layers and owns global configuration, assets, and the ordered migration history. The host should stay thin and should not import feature-specific menus, pages, or server handlers.

### Portal core

The foundational layer that owns the application shell, sessions, active-organization context, authorization adapters, feature registry, shared dashboard, and core database contracts. Feature layers depend on portal core; portal core does not depend on a business feature.

### Platform layer

A layer that provides reusable portal infrastructure rather than one optional business capability. Authentication, account and organization management, administration, and the public site are current platform layers.
...
One item a feature adds to a shared portal surface, such as a navigation link, module, module-menu item, or dashboard widget. The host aggregates contributions without importing the feature directly.

### Feature registry

Portal core’s runtime collection of feature definitions. Registration is idempotent by feature ID: registering the same ID again replaces its previous definition, enabling capability-filtered menus and widgets.
...
A typed mapping from organization roles to feature actions. Each server route independently enforces the relevant action through portal-core authorization adapters.
...
A feature-owned Vue component placed by portal core into `attention`, `main`, or `aside`. Its definition also declares size and deterministic order. An error boundary isolates each widget from the rest of the dashboard.
...
A PostgreSQL namespace containing a layer’s tables and enums. Its name derives from the layer name with hyphens replaced by underscores. Cross-schema foreign keys may reference portal-core users and organizations.

### Ordered migration history

The single root Drizzle migration sequence covering portal core and every enabled feature layer. Feature code owns its schema declarations; the host owns deployment order and migration review.
...
A stable portal-core function used by feature APIs for sessions, active organizations, authorization, organization members, organization lookup, and database access. It prevents runtime dependencies on private authentication or host implementations.
...
A feature layer distributed as source and placed directly below the consuming portal’s `layers/` directory. This is Customer Portal’s preferred contribution shape today.

### Remote layer

A Nuxt layer loaded from a pinned Git source through `extends`. Nuxt supports the mechanism; the layer author owns Customer Portal compatibility and migration integration.

### Packaged layer

A separately versioned Nuxt layer published to a package registry and loaded through `extends`. Customer Portal does not yet publish a stable layer SDK or built-in feature packages.
