...
Generate `BETTER_AUTH_SECRET` and `PORTAL_ENCRYPTION_KEY` separately with `openssl rand -base64 32`; never copy a documentation placeholder into a deployment. See the [Better Auth secret reference](https://better-auth.com/docs/reference/options#secret) for the upstream authentication contract.
