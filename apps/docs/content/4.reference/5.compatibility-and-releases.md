...
The maintained installation example targets `0.3.0`. Check the [published releases](https://github.com/ludulicious/nuxt-customer-portal/releases) and npm availability for the exact version before installing. A version in the source tree is not evidence of a completed npm publication.

Timesheets and invoicing are used by the maintainers' software agency. Version `0.3.0` is the first release intended for installation from npm. Evaluate compatibility with your host, deployment, and custom modules before upgrading. Service Requests is an optional example extension for developers.
