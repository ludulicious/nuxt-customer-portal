...
githubPath: apps/docs/content/2.architecture/4.tenancy-and-security.md
navigation:
...
export default defineEventHandler(async (event) => {
  const { session, organizationId } = await requireFeatureAccess(event, exampleFeature.policy, 'read')
