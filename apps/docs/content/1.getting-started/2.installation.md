...
The preset contains core, the neutral UI fallback, authentication, OWNER organizations, and generic Clients. Add optional business packages explicitly.
...
npx nuxt-customer-portal owner seed --organization-name Acme --organization-slug acme --user-name "Portal Owner" --user-email owner@example.com --user-password '<strong-password>'
```

`doctor` also validates configured default client modules. Migrations execute in dependency order under a PostgreSQL advisory lock. The OWNER seed is idempotent. For an existing pre-refactor database, first run `clients migrate --owner <id-or-slug>` as a dry run; applying it additionally requires `--apply --backup-confirmed`.
