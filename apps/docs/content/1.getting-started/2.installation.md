...
The command below installs the published `0.3.3` release. The kit generates a project with matching versions of the portal packages, including `@nuxt-customer-portal/saas-configuration`. See [compatibility and releases](/reference/compatibility-and-releases).
...
```bash [pnpm]
pnpm dlx @nuxt-customer-portal/kit@0.3.3 init my-portal
```

```bash [Yarn]
yarn dlx @nuxt-customer-portal/kit@0.3.3 init my-portal
```

```bash [Bun]
bunx @nuxt-customer-portal/kit@0.3.3 init my-portal
```
