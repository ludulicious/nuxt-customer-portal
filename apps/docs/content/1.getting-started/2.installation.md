...
The command below installs the release currently published under npm's `latest` tag. The kit generates a project with matching versions of the portal packages, including `@nuxt-customer-portal/saas-configuration`. See [compatibility and releases](/reference/compatibility-and-releases).
...
```bash [pnpm]
pnpm dlx @nuxt-customer-portal/kit@latest init my-portal
```

```bash [Yarn]
yarn dlx @nuxt-customer-portal/kit@latest init my-portal
```

```bash [Bun]
bunx @nuxt-customer-portal/kit@latest init my-portal
```
