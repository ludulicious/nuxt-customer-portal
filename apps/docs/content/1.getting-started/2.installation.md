...
The command below targets `0.1.0-alpha.0`. It requires that version of the kit and the portal packages, including `@nuxt-customer-portal/saas-configuration`, to be published. Until then, contributors can run the command from the source checkout as described below. See [compatibility and releases](/reference/compatibility-and-releases).
...
```bash [pnpm]
pnpm dlx @nuxt-customer-portal/kit@0.1.0-alpha.0 init my-portal
```

```bash [Yarn]
yarn dlx @nuxt-customer-portal/kit@0.1.0-alpha.0 init my-portal
```

```bash [Bun]
bunx @nuxt-customer-portal/kit@0.1.0-alpha.0 init my-portal
```
...
The source command generates the same project as the packaged CLI. Before the npm release, maintainers must install locally packed packages to test the generated application. See the [release checklist](https://github.com/ludulicious/nuxt-customer-portal/blob/master/apps/docs/README.md#release-checklist).
