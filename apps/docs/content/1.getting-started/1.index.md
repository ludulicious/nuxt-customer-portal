---
title: Introduction
description: Understand what Customer Portal provides, how it is structured, and where to begin.
githubPath: apps/docs/content/1.getting-started/1.index.md
navigation:
  icon: i-lucide-house
---

Nuxt Customer Portal is an MIT-licensed collection of Nuxt layers for organizations that need a secure place to work with customers, suppliers, and internal teams. It provides the shared foundation—authentication, organizations, roles, administration, navigation, and a dashboard—while business workflows are added as optional packages.

The repository now also includes `apps/saas-portal`, a ready-to-run application for a single organization. Its guided setup lets an administrator choose the portal name and branding, appearance, enabled modules, English and Dutch homepage content, support details, and legal pages without building a custom host first.

::callout{icon="i-lucide-cloud" color="primary"}
#title
Request a hosted evaluation

#description
Want to evaluate the configurable SaaS portal without installing it? [Request a hosted evaluation version at PortalNuxt](https://portalnuxt.com).
::

::warning
The repository is licensed under MIT. Before depending on the alpha packages, read the current [licensing and compatibility status](/reference/compatibility-and-releases).
::

That division is the project’s main design choice:

- **Portal core** owns stable contracts and cross-cutting infrastructure.
- **Feature layers** own complete business capabilities, from pages to database tables.
- **The host application** stays thin and composes the installed layers.

::note
Timesheets and Invoices are independent production features. Install the optional invoice-timesheets bridge when approved time should be available as an invoice source.
::

## Choose your path

::card-group
:::card
---

icon: i-lucide-rocket
title: Run it locally
to: /getting-started/installation
---

Install Customer Portal, configure PostgreSQL, and start the development server.
:::

:::card
---

icon: i-lucide-cloud
title: Evaluate it online
to: https://portalnuxt.com
target: _blank
---

Request a hosted version configured for your organization.
:::

:::card
---

icon: i-lucide-waypoints
title: Understand the architecture
to: /architecture/overview
---

Learn how portal core, the host, and feature layers fit together.
:::

:::card
---

icon: i-lucide-library
title: Look up a contract
to: /reference
---

Find configuration, feature-registry fields, OpenAPI discovery, and compatibility guidance.
:::

:::card
---

icon: i-lucide-clock-3
title: Explore the modules
to: /modules/timesheets
---

See how the production Timesheets workflow is assembled, then explore the independent Invoices module.
:::

:::card
---

icon: i-lucide-blocks
title: Build a feature layer
to: /contributing/create-a-layer
---

Add a self-contained module without coupling it to the host application.
:::

:::card
---

icon: i-lucide-palette
title: Brand a deployment
to: /getting-started/customization
---

Replace the reference identity, public content, themes, and email branding.
:::
::

## What the base portal includes

- Email/password authentication, email verification, password recovery, and optional GitHub or Google OAuth.
- Organizations, memberships, invitations, active-organization selection, and owner/admin/member roles.
- System-administrator tools for managing users and organizations.
- A feature registry for navigation, module menus, dashboard widgets, and typed policies.
- PostgreSQL with Drizzle ORM and dependency-ordered provider migration streams.
- English and Dutch localization with messages owned by each layer.
- A Docker-ready Nuxt application with frontend and server routes in one service.

## Current feature layers

| Layer                | Purpose                                                                              |
| -------------------- | ------------------------------------------------------------------------------------ |
| `core`               | Headless sessions, authorization, database access, registry, and extension contracts |
| `ui`                 | Neutral shell primitives, fallback layouts, dashboards, and surface rendering        |
| `authentication`     | Sign in, sign up, verification, and password recovery                                |
| `organizations`      | Account settings, organizations, members, and invitations                            |
| `administration`     | System-administrator user and organization management                                |
| `preset`             | Core platform composition for common installations                                   |
| `kit`                | Configuration, diagnostics, migrations, and legacy adoption                          |
| `service-requests`   | A compact reference feature for tenant-scoped request workflows                      |
| `timesheets`         | Time entry, approvals, suppliers, and reporting                                      |
| `invoices`           | Free-form invoices, PDFs, delivery, payments, and client access                      |
| `invoice-timesheets` | Optional approved-time invoice source bridge                                         |

For custom implementations, public pages, branding, and the application shell belong to the consuming host, as demonstrated independently by Apex and Brutal. The included `apps/saas-portal` provides a configurable alternative for organizations that want a ready-to-run host.

The source is available on [GitHub](https://github.com/ludulicious/nuxt-customer-portal). If you want to contribute, start with the [contribution guide](/contributing).
