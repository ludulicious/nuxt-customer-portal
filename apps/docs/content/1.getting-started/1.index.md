...
githubPath: content/1.getting-started/1.index.md
navigation:
  icon: i-lucide-house
---

Customer Portal is a publicly developed Nuxt application for organizations that need a secure place to work with customers, suppliers, and internal teams. It is intended for an explicit open-source release, but the product license is still pending. It provides the shared foundation—authentication, organizations, roles, administration, navigation, and a dashboard—while business workflows are added as Nuxt layers.

::warning
Public source does not replace a software license. Before using or redistributing Customer Portal outside an authorized deployment, read the current [licensing and compatibility status](/reference/compatibility-and-releases).
::
...
::card-group
  ...
  to: /modules/timesheets-invoices
  ---
...
- PostgreSQL with Drizzle ORM and a single ordered migration history.
...
| Layer | Purpose |
| --- | --- |
| `portal-core` | Shell, sessions, authorization, database access, dashboard aggregation, and extension contracts |
| `authentication` | Sign in, sign up, verification, and password recovery |
| `account-organizations` | Account settings, organizations, members, and invitations |
| `administration` | System-administrator user and organization management |
| `public-site` | Public pages, contact, blog content, and assets |
...
The source is available on [GitHub](https://github.com/ludulicious/customer-portal). If you want to contribute, start with the [contribution guide](/contributing).
