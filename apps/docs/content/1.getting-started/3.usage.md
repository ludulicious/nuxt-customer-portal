...
Saved provider credentials take precedence over the environment fallback. Set `PORTAL_ENCRYPTION_KEY` before storing credentials through Admin > Email. Invoice sender details remain in `/admin/invoices/settings`; their sender domain must be verified with the configured provider.
