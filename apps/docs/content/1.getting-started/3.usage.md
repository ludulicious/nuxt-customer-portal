...
githubPath: apps/docs/content/1.getting-started/3.usage.md
navigation:
...
Authentication messages, invitations, timesheet notifications, and invoice delivery use the shared portal email provider. Configure it under **Admin > Email** at `/admin/email`, or supply the environment fallback:
...
Saved provider credentials take precedence over the environment fallback. Set `PORTAL_EMAIL_ENCRYPTION_KEY` before storing credentials through Admin > Email. Invoice sender details remain in `/admin/invoices/settings`; their sender domain must be verified with the configured provider.

Test delivery to a controlled recipient before inviting users or sending invoices. The seeded evaluation owner can sign in without email, but invitations and newly registered accounts depend on working delivery.
...
Disabling a layer removes its routes and registrations, but it does not delete its database schema or stored data. See [Nuxt layers](/architecture/layers).
